...
1. Right-click the **`/apps/mywebsite`** folder and click **Create** > **Create Folder**. Geben Sie im Dialogfeld **Ordner erstellen** als Ordnernamen `components` ein und klicken Sie auf **OK**.
1. Right-click the **`/apps/mywebsite`** folder and click **Create** > **Create Folder**. Geben Sie im Dialogfeld **Ordner erstellen** als Ordnernamen `templates` ein und klicken Sie auf **OK**.
...
1. [Kopieren Sie mithilfe von WebDAV](/help/sites-administering/webdav-access.md) -Zugriff auf die URL unter https://localhost:4502 die Beispieldatei `static.css` und den `images` -Ordner aus der heruntergeladenen Datei &quot;mywebsite.zip&quot;in den `/etc/designs/mywebsite` Ordner.
...
1. To specify the paths of the pages that can use this template, click the plus button and type `/content(/.*)?` in the text box that appears. Klicken Sie dann auf **Weiter**.
...
1. Click **Next** until the Allowed Children panel of the dialog appears, and then click **OK**. Klicken Sie in CRXDE Lite auf **Alle speichern**.
...
1. Klicken Sie auf **Erstellen**. In the folder tree, select the **/Websites/My Website** page and click **New** > **New Page**.
...
1. To link your pages to the mywebsite design, in CRXDE Lite, select the `/content/mywebsite/en/jcr:content` node. Geben Sie auf der Registerkarte „Eigenschaften“ die folgenden Werte für eine neue Eigenschaft ein und klicken Sie dann auf „Hinzufügen“:
...
1. On the Basic tab of the dialog, expand **More Titles and Description,** and for the **Subtitle** property, type **what we do**. Klicken Sie auf **OK**.
...
1. As Path of list root, enter: `/content/mywebsite/en`. Klicken Sie auf „OK“. Die listchildren-Komponente auf Ihrer Seite sieht nun wie folgt aus:
...
1. Under the `design_dialog/items/items`node, create a new node named `img` of type `cq:Widget`. Fügen Sie die folgenden Eigenschaften hinzu und klicken Sie auf Alle speichern:
