...
The **AND Split** creates a split in the workflow, after which both branches will be active. Sie fügen jeder Verzweigung nach Bedarf Workflow-Schritte hinzu. Mit diesem Schritt können Sie mehrere Prozesspfade in einem Workflow einrichten. Sie können z. B. mehrere Bewertungsschritte parallel zulassen, sodass Sie Zeit sparen.
...
Mit dem **Goto-Schritt** können Sie den nächsten Schritt angeben, der im Workflow-Modell ausgeführt werden soll. Sie können eine Regeldefinition, ein externes Skript oder ein ECMA-Skript als Routingausdruck angeben, um den nächsten Schritt für das Workflow-Modell auszuwerten.

* Wenn die angegebene Bedingung &quot;true&quot;lautet, wird der **Goto-Schritt** abgeschlossen und die Workflow-Engine führt den angegebenen Schritt aus.
* Wenn die angegebene Bedingung nicht &quot;true&quot;enthält, wird der **Goto-Schritt** abgeschlossen und die normale Routinglogik bestimmt den nächsten auszuführenden Schritt.

Mit **Zum Schritt wechseln** können Sie erweiterte Routingstrukturen im Workflow-Modell implementieren. For example, to implement a loop, the **Goto Step** can be defined to execute a prior step in the workflow, with the routing expression evaluating a loop condition.
...
* **Prozess**
   ...
   * **Zielschritt**: Wählen Sie den auszuführenden Schritt aus, nachdem Sie die Bedingung für den Routingausdruck ausgewertet haben.
   * **Routing-Ausdruck**: Wählen Sie &quot;Regeldefinition&quot;, &quot;Externes Skript&quot;oder ein ECMA-Skript, das bestimmt, ob der **Target-Schritt** ausgeführt werden soll.
...
Sie können auch eine for-Schleife simulieren, indem Sie &quot;Regeldefinition&quot;als Routingausdruck verwenden. [Erstellen Sie eine **Zählvariable** des Datentyps](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) &quot;Lang&quot;. Verwenden Sie den **Ausdruck** als Zuordnungsmodus im Schritt **[Variable](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)**festlegen, um den Wert der **Zählvariable**auf **Zählung + 1**bei jeder Ausführung des **Setvariablenschritts**festzulegen.

![Simulieren einer for-Schleife](assets/variable_use_case_count_new.png)

Verwenden Sie im **Goto-Schritt** die **Set-Variable** als **Target-Schritt** und **zählen Sie &lt; 5** als Routingausdruck.

![Bedingung für die Simulation einer for-Schleife](assets/variable_use_case_count1_new.png)

Der Schritt **Variable** festlegen wird wiederholt ausgeführt, um den Wert der **** Zählvariable bei jeder Ausführung um 1 zu erhöhen, bis der Wert 5 erreicht.
...
* Bearbeiten Sie die **ODER-Verzweigungs-Eigenschaften**:
   ...
   * **Zweige (*x)***
      ...
      * **** Zweig hinzufügen: Fügen Sie dem Schritt weitere Verzweigungen hinzu.
      * **Routing-Ausdruck** auswählen: Wählen Sie den Routingausdruck aus, um die aktive Verzweigung auszuwerten. Mögliche Werte sind: Regeldefinition, externes Skript und ECMA-Skript.
      * **Klicken, um Ausdruck** hinzuzufügen: Fügen Sie zum Auswerten der aktiven Verzweigung einen Ausdruck hinzu, wenn Sie als Routingausdruck &quot; **Regeldefinition** &quot;auswählen.
      * **Skriptpfad**: Der Pfad zu einer Datei, die das Skript zum Auswerten der aktiven Verzweigung enthält, wenn Sie als Routingausdruck &quot; **Externes Skript** &quot;auswählen.
      * **Skript**: Fügen Sie das Skript in das Feld ein, um die aktive Verzweigung auszuwerten, wenn Sie **ECMA-Skript** als Routingausdruck auswählen.
   ...
   >[!NOTE]
   >
   >    * Eine Verzweigung wird zu einem bestimmten Zeitpunkt basierend auf dem Routingausdruck ausgewertet.
...
>[!NOTE]
>
>Einige Eigenschaften müssen konfiguriert werden, um E-Mail-Benachrichtigungen zu aktivieren. Sie können die E-Mail-Vorlage auch anpassen oder eine E-Mail-Vorlage für eine neue Sprache hinzufügen. See [Configuring Email Notification](/help/sites-administering/notification.md#configuringemailnotification) to configure email notifications in AEM.
...
1. **Eigenschaft „Dialogpfad“**
   ...
   The **Dialog Participant Step** has the **Dialog Path** property (together with the properties of a [Participant Step](#participant-step)). Der Wert der Eigenschaft **Dialogpfad** entspricht dem Pfad zum Knoten `dialog` im Dialogfeld.
...
* **OSGi-Dienst**
   ...
   Services must implement the [com.day.cq.workflow.exec.ParticipantStepChooser](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/ParticipantStepChooser.html) interface. Die Schnittstelle definiert die folgenden Mitglieder:
   ...
   * `getParticipant` Methode: Gibt die dynamisch aufgelöste Principal-ID als `String` Wert zurück.
   ...
   >
   >
   >Eine Gruppen-ID kann jedoch nur für eine Komponente **Teilnehmer-Schritt** verwendet werden, wenn eine Liste der Teilnehmer zurückgegeben wird. For a **Dynamic Participant Step** an empty list is returned and this cannot be used for delegation.
...
Die folgende Java-Klasse implementiert die `ParticipantStepChooser`-Oberfläche. Die Klasse gibt den Namen des Teilnehmers zurück, der den Workflow initiiert hat. The code uses the same logic that the sample script (`initiator-participant-chooser.ecma`) uses.
...
* Für die Komponente **Start des Formulars** muss die Eigenschaft **Aktionstyp** auf `Edit Workflow Controlled Resource(s)`eingestellt sein.
...
* The form must include a **Workflow Submit Button(s)** component. Sie konfigurieren keine Eigenschaften für die Komponente.
...
### Variable einstellen - Konfiguration {#setvariable}
