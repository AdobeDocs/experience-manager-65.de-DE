...
Sie können zwischen folgenden Optionen wählen: **Aus**, **Sofort**, **1 h**, **6h**, **12 Std.**, **24 Std.**.
...
* **Benutzer/Gruppe**
   ...
   * Wenn Sie den Schritt einer ganzen Gruppe zuweisen, haben alle Benutzer dieser Gruppe die Aktion in ihrer **Workflow-Posteingang**.
...
Die **Zum Schritt wechseln** ermöglicht Ihnen, den nächsten Schritt anzugeben, der im Workflow-Modell ausgeführt werden soll. Sie können eine Regeldefinition, ein externes Skript oder ein ECMA-Skript als Routing-Ausdruck angeben, um den nächsten Schritt für das Workflow-Modell zu bewerten.

* Wenn die von Ihnen angegebene Bedingung &quot;true&quot;lautet, wird die **Zum Schritt wechseln** wird abgeschlossen und die Workflow-Engine führt den angegebenen Schritt aus.
* Wenn die von Ihnen angegebene Bedingung nicht &quot;true&quot;lautet, wird die **Zum Schritt wechseln** completes und die normale Routing-Logik bestimmt den nächsten auszuführenden Schritt.

Mit **Zum Schritt wechseln** können Sie erweiterte Routingstrukturen im Workflow-Modell implementieren. Um beispielsweise eine Schleife zu implementieren, muss die **Zum Schritt wechseln** kann definiert werden, um einen vorherigen Schritt im Workflow auszuführen, wobei der Routing-Ausdruck eine Schleifenbedingung auswertet.
...
* **Prozess**
   ...
   * **Routing-Ausdruck**: Wählen Sie Regeldefinition, externes Skript oder ein ECMA-Skript aus, das bestimmt, ob die **Target-Schritt**.

      * **Regeldefinition:** Verwenden Sie die [Ausdruckseditor](/help/forms/using/variable-in-aem-workflows.md#use-expression-editor) , um die Regel zu definieren.
      * **Externes Skript:** Der Pfad des externen Skripts.
      * **ECMA-Skript**: Das Skript, das bestimmt, ob die **Zum Schritt wechseln**.
...
Sie können auch eine für eine Schleife simulieren, indem Sie die Regeldefinition als Routing-Ausdruck verwenden. [Erstellen Sie eine **count** Variable](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) des Datentyps Long . Verwendung **Ausdruck** als Zuordnungsmodus im **[Variable festlegen](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)** Schritt zum Festlegen des Werts für **count** zu **count + 1** bei jeder Ausführung der **Variable festlegen** Schritt.

![Simulieren einer for-Schleife](assets/variable_use_case_count_new.png)

Im **Zum Schritt wechseln**, verwenden **Variable festlegen** als **Target-Schritt** und **count &lt; 5** als Routing-Ausdruck.

![Bedingung für die Simulation einer for-Schleife](assets/variable_use_case_count1_new.png)

Die **Variable festlegen** -Schritt führt wiederholt aus, um den Wert von **count** bei jeder Ausführung um 1 erhöht, bis der Wert 5 erreicht.
...
* Bearbeiten Sie die **ODER-Verzweigungs-Eigenschaften**:
   ...
   * **Verzweigungen (*x)***
      ...
      * **Klicken Sie auf Ausdruck hinzufügen**: Fügen Sie einen Ausdruck hinzu, um die aktive Verzweigung auszuwerten, wenn Sie **Regeldefinition** als Routing-Ausdruck.
      * **Skriptpfad**: Der Pfad zu einer Datei, die das Skript zum Auswerten der aktiven Verzweigung enthält, wenn Sie **Externes Skript** als Routing-Ausdruck.
      * **Skript**: Fügen Sie das Skript in das Feld ein, um die aktive Verzweigung auszuwerten, wenn Sie **ECMA-Skript** als Routing-Ausdruck.
...
* **Dialogfeld**
   ...
   * **Dialogpfad**: Der Pfad zum Dialogfeldknoten des [Dialogfeld erstellen](#dialog-participant-step-creating-a-dialog).
...
1. **Eigenschaft „Dialogpfad“**
   ...
   Die **Dialogfeld &quot;Teilnehmer - Schritt&quot;** hat die **Dialogpfad** -Eigenschaft (zusammen mit den Eigenschaften eines [Teilnehmer-Schritt](#participant-step)). Der Wert der Eigenschaft **Dialogpfad** entspricht dem Pfad zum Knoten `dialog` im Dialogfeld.
...
* **ECMA-Skript**
   ...
   >[!NOTE]
   >
   >Die **Workflow-Initiator-Teilnehmerauswahl** -Komponente erweitert die **Dynamischer Teilnehmer - Schritt** und verwendet dieses Skript als Schrittimplementierung.

* **OSGi-Dienst**

   Dienste müssen die [com.day.cq.workflow.exec.ParticipantStepChooser](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/ParticipantStepChooser.html) -Schnittstelle. Die Schnittstelle definiert die folgenden Mitglieder:
   ...
   * `getParticipant` -Methode: Gibt die dynamisch aufgelöste Prinzipal-ID als `String` -Wert.
   ...
   >
   >
   >Eine Gruppen-ID kann jedoch nur für eine Komponente **Teilnehmer-Schritt** verwendet werden, wenn eine Liste der Teilnehmer zurückgegeben wird. Für **Dynamischer Teilnehmer - Schritt** eine leere Liste zurückgegeben wird, die nicht für die Zuweisung verwendet werden kann.
...
Die folgende Java-Klasse implementiert die `ParticipantStepChooser`-Oberfläche. Die Klasse gibt den Namen des Teilnehmers zurück, der den Workflow initiiert hat. Der Code verwendet dieselbe Logik wie das Beispielskript (`initiator-participant-chooser.ecma`) verwendet.
...
Im **Dynamischer Teilnehmer - Schritt** Eigenschaftendialogfeld, die **Teilnehmerauswahl** -Liste enthält das Element `Workflow Initiator Participant Chooser (script)`, der diesen Dienst darstellt.
...
* **Formular**
   ...
   * **Formularpfad**: Der Pfad zum [Formular erstellen](#form-participant-step-creating-the-form).
...
* Die **Beginn des Formulars** -Komponente muss die **Aktionstyp** Eigenschaft auf `Edit Workflow Controlled Resource(s)`.
* Die **Beginn des Formulars** -Komponente muss einen Wert für die `Form Identifier` -Eigenschaft.
...
* Das Formular muss eine **Workflow-Senden-Schaltfläche(n)** -Komponente. Sie konfigurieren keine Eigenschaften für die Komponente.

Die Workflow-Anforderungen bestimmen, wo Felddaten gespeichert werden sollen. Beispielsweise können Felddaten verwendet werden, um die Eigenschaften von Seiteninhalten zu konfigurieren. Der folgende Wert eines **Elementname** -Eigenschaft speichert Felddaten als Wert der `redirectTarget` -Eigenschaft der `jcr:content` node:
