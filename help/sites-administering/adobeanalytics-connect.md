...
Um AEM Daten in Adobe Analytics verfolgen zu können, benötigen Sie ein gültiges Adobe Experience Cloud Adobe Analytics-Konto.
...
* [Adobe Experience Cloud-Anmeldung](https://experience.adobe.com/#/@login/home)
...
Adobe Analytics [Rechenzentren](https://experienceleague.adobe.com/docs/analytics/analyze/reports-analytics/reporting-interface/overview-data-collection.html?lang=en) Daten erfassen, verarbeiten und speichern, die mit Ihrer Adobe Analytics Report Suite verknüpft sind. Konfigurieren Sie AEM für die Verwendung des Rechenzentrums, das Ihre Adobe Analytics Report Suite hostet. Das Rechenzentrum wird in Ihrem Vertrag erwähnt. Wenden Sie sich für diese Informationen an einen Administrator in Ihrem Unternehmen.

Verwenden Sie bei Bedarf Folgendes, um zum richtigen Rechenzentrum weitergeleitet zu werden: `https://api.omniture.com/`.

Wenn Ihr Unternehmen eine Datenerfassung oder einen Datenabruf von einem bestimmten Rechenzentrum benötigt, verwenden Sie Folgendes:
...
## Erstellen eines Adobe Analytics-Frameworks {#creating-a-adobe-analytics-framework}
...
1. Im **Framework erstellen** dialog:
...
1. Klicken Sie im Abschnitt **Report Suites** des seitlichen Pods (rechts neben dem Hauptbedienfeld) auf **Element hinzufügen**. Wählen Sie dann über das Dropdown-Menü die Report Suite-ID aus (z. B. `geometrixxauth`), mit dem das Framework interagiert.
...
* **Tracking-Server**
   ...
   * Enthält die URL, die zum Senden von Adobe Analytics-Aufrufen verwendet wird
      ...
      * `d1` - dem Rechenzentrum entspricht, an das die Informationen gesendet werden (entweder `d1`, `d2`oder `d3`)
...
* **Besucher-Namespace**
   ...
   * Ändern Sie beispielsweise den Namespace in **CNAME** bewirkt, dass die Aufrufe an Adobe Analytics wie folgt aussehen **CNAME.d1.omtrdc.net** anstelle der Standardeinstellung.

## Verknüpfen einer Seite mit einem Adobe Analytics Framework {#associating-a-page-with-a-adobe-analytics-framework}
...
1. Die Dropdown-Auswahl für **Adobe Analytics** an die verfügbaren Optionen angehängt. Wählen Sie die erforderliche Framework-Konfiguration aus.

1. Auswählen **Speichern und schließen**.
1. So aktivieren Sie die Seite und alle verbundenen Konfigurationen/Dateien: **[Veröffentlichen](/help/sites-authoring/publishing-pages.md)** die Seite.
1. Der letzte Schritt besteht darin, die Seite in der Veröffentlichungsinstanz zu besuchen und nach einem Keyword (z. B. Aubergine) zu suchen, indem Sie die **Suche** -Komponente.
1. Sie können dann Aufrufe an Adobe Analytics mit einem entsprechenden Tool überprüfen. Zum Beispiel mit [Adobe Experience Cloud Debugger](https://experienceleague.adobe.com/docs/experience-platform/debugger/home.html).
...
Navigieren Sie wie beim Erstellen einer Adobe Analytics-Konfiguration oder eines Adobe Analytics-Frameworks zum (veralteten) Bildschirm **Cloud-Services**. Auswählen **Konfigurationen anzeigen** und klicken Sie dann auf den Link zur spezifischen Konfiguration, die Sie aktualisieren möchten.

Drücken Sie beim Bearbeiten einer Adobe Analytics-Konfiguration die **Bearbeiten** , wenn auf der Konfigurationsseite selbst das **Komponente bearbeiten** angezeigt.
