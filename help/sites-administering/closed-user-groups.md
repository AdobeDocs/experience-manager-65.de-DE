...
* Es werden weder Schreibberechtigungen noch Berechtigungen verarbeitet, die für die Änderung von geschützten JCR-Inhalten erforderlich sind (Zugriffssteuerung, Informationen zum Node-Typ, Versionierung, Sperren oder Benutzerverwaltung unter anderem); Diese Berechtigungen sind von einer CUG-Richtlinie nicht betroffen und werden nicht vom zugehörigen Autorisierungsmodell bewertet. Ob diese Berechtigungen gewährt werden, hängt von den anderen Modellen ab, die in der Sicherheitseinrichtung konfiguriert sind.
...
Die folgenden Schritte sind erforderlich, um eine vorhandene CUG-Richtlinie zu bearbeiten. Please note that the modified policy needs to written back and changes needs to be persisted using `javax.jcr.Session.save()`.
...
Hier finden Sie Schritte zum Entfernen einer vorhandenen Authentifizierungspflicht. The requirement will only be unregistered from the Apache Sling Authenticator if the `RequirementHandler` has been configured for the tree containing the target node.
...
| **Authentifizierung erforderlich** | **Anmeldepfad** | **Eingeschränkter Lesezugriff** | **Erwartete Auswirkung** |
|---|---|---|---|
| Ja | Ja | Ja | Ein bestimmter Benutzer kann die mit der CUG-Richtlinie markierte Unterstruktur nur anzeigen, wenn eine effektive Berechtigungsbewertung den Zugriff erlaubt. Ein nicht authentifizierter Benutzer wird zur angegebenen Anmeldeseite weitergeleitet. |
...
Beachten Sie, dass das Platzieren der CUG-Richtlinie nur am Knoten jcr:content unterhalb einer bestimmten Seite den Zugriff auf den Inhalt s.str einer bestimmten Seite beschränkt, jedoch nicht auf Geschwister oder untergeordnete Seiten wirksam wird. Dies kann ein gültiger Anwendungsfall sein. Sie erreichen dies mithilfe eines Repository-Editors, der die Anwendung detaillierter Zugriffssteuerungsinhalte erlaubt. Es steht jedoch im Gegensatz zur früheren Implementierung, bei der die Platzierung einer cq:cugEnabled-Eigenschaft auf dem Knoten jcr:content intern dem Seitenknoten neu zugeordnet wurde. Diese Zuordnung wird nicht mehr durchgeführt.

**Berechtigungsprüfung mit CUG-Richtlinien**

Die Umstellung von der alten CUG-Unterstützung auf ein zusätzliches Autorisierungsmodell verändert die Art und Weise, wie wirksame Leseberechtigungen geprüft werden. As described in the [Jackrabbit documentation](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html), a given principal allowed to view the `CUGcontent` will only be granted read access if the permission evaluation of all models configured in the Oak repository grant read access.
