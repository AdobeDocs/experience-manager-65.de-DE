...
description: Beschreibt, wie Assets in [!DNL Adobe Experience Manager], wenden Sie Metadaten an, generieren Sie Ausgabedarstellungen und aktivieren Sie sie für Veröffentlichungsinstanzen.
contentOwner: AG
...
Beim Migrieren von Assets in [!DNL Adobe Experience Manager], sind mehrere Schritte zu beachten. Das Extrahieren von Assets und Metadaten aus ihrer aktuellen Startseite fällt nicht in den Geltungsbereich dieses Dokuments, da es zwischen den Implementierungen stark variiert. In diesem Dokument wird jedoch beschrieben, wie diese Assets in [!DNL Experience Manager], wenden Sie ihre Metadaten an, generieren Sie Ausgabedarstellungen und aktivieren Sie sie für Veröffentlichungsinstanzen.

## Voraussetzungen {#prerequisites}

Bevor Sie einen der Schritte in dieser Methode tatsächlich durchführen, überprüfen und implementieren Sie die Anleitungen unter [Tipps zur Leistungsoptimierung von Assets](performance-tuning-guidelines.md). Viele dieser Schritte, etwa die Konfiguration einer maximalen Anzahl gleichzeitiger Aufträge, führt zu einer deutlich höheren Stabilität und Leistung der Server unter Last. Andere Schritte wie die Konfiguration eines Dateidatenspeichers erweisen sich als wesentlich schwieriger, nachdem Assets in das System geladen wurden.

>[!NOTE]
>
>Die folgenden Asset-Migrationstools sind nicht Teil von [!DNL Experience Manager] und werden von Adobe nicht unterstützt:
>
...
## Migrieren zu [!DNL Experience Manager] {#migrating-to-aem}
...
Deaktivieren Sie vor Beginn der Migration die Starter für die [!UICONTROL DAM-Update-Asset] Arbeitsablauf. Am besten nehmen Sie zunächst alle Assets in das System auf und führen dann die Workflows stapelweise aus. Wenn Ihr System bereits „live“ ist, während die Migration durchgeführt wird, können Sie diese Aktivitäten so planen, dass sie außerhalb der Arbeitszeiten ausgeführt werden.

### Laden von Tags {#loading-tags}

Womöglich verfügen Sie bereits über eine Tag-Taxonomie für Ihre Bilder. Bei Tools wie dem CSV Asset Importer und [!DNL Experience Manager] Unterstützung für Metadatenprofile kann das Anwenden von Tags auf Assets automatisieren. Die Tags müssen in das System geladen werden. Mit [Tag Maker von ACS AEM-Tools](https://adobe-consulting-services.github.io/acs-aem-tools/features/tag-maker/index.html) können Sie Tags mithilfe einer in das System geladenen Microsoft Excel-Tabelle auffüllen.
...
Nachdem Sie die Assets in das System geladen haben, müssen Sie sie über die [!UICONTROL DAM-Update-Asset] Workflow zum Extrahieren von Metadaten und Generieren von Ausgabeformaten. Bevor Sie diesen Schritt durchführen, müssen Sie die [!UICONTROL DAM-Update-Asset] an Ihre Anforderungen anzupassen. Der vordefinierte Workflow enthält viele Schritte, die für Sie möglicherweise nicht erforderlich sind, z. B. die Erstellung von Dynamic Media-PTIFF-Dateien oder [!DNL InDesign Server] Integration.
...
Sobald wir die Migration abgeschlossen haben, werden die Starter für die [!UICONTROL DAM-Update-Asset] Workflows sollten erneut aktiviert werden, um die Ausgabegenerierung und Metadatenextraktion für die laufende tägliche Systemnutzung zu unterstützen.

## Migrieren von [!DNL Experience Manager] Bereitstellungen {#migrating-between-aem-instances}

Obwohl es nicht so häufig vorkommt, müssen Sie manchmal große Datenmengen von einem [!DNL Experience Manager] Verlegung in einen anderen Wenn Sie beispielsweise eine [!DNL Experience Manager] Aktualisierung, Aktualisierung Ihrer Hardware oder Migration zu einem neuen Rechenzentrum, beispielsweise mit einer AMS-Migration.

In diesem Fall sind die Assets schon mit Metadaten aufgefüllt und Wiedergaben sind bereits generiert. Sie können sich einfach darauf konzentrieren, Assets zwischen Instanzen zu verschieben. Bei der Migration zwischen [!DNL Experience Manager] -Implementierung führen Sie die folgenden Schritte aus:

1. Workflows deaktivieren: Da Sie Ausgabeformate zusammen mit unseren Assets migrieren, möchten Sie die Workflow-Starter für [!UICONTROL DAM-Update-Asset] Arbeitsablauf.

1. Tags migrieren: Da bereits Tags in der Quelle geladen wurden [!DNL Experience Manager] -Implementierung können Sie sie in einem Inhaltspaket erstellen und das Paket auf der Zielinstanz installieren.

1. Migrieren von Assets: Es werden zwei Tools zum Verschieben von Assets von einem [!DNL Experience Manager] Implementierung in eine andere:
...
1. Aktivieren von Assets: Befolgen Sie die Anweisungen für [Aktivieren von Assets](#activating-assets) dokumentiert für die Erstmigration zu [!DNL Experience Manager].
...
1. Aktivieren Sie Workflows: Nachdem Sie die Migration abgeschlossen haben, aktivieren Sie die Starter für die [!UICONTROL DAM-Update-Asset] -Arbeitsablauf zur Unterstützung der Ausgabegenerierung und Metadatenextraktion für die laufende tägliche Systemnutzung.
