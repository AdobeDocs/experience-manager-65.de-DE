...
1. Navigieren Sie zum Bedienfeld &quot;Filter&quot;in der [!DNL Assets]-Benutzeroberfläche. Das Prädikat **[!UICONTROL Mehrwert-Eigenschaft]** wird zum Bereich hinzugefügt.
...
Mit der Tag-Prognose können Sie tagbasierte Suchen nach Assets durchführen. Standardmäßig sucht [!DNL Assets] Assets anhand der angegebenen Tags nach Übereinstimmungen mit einem oder mehreren Tags. Das heißt, die Suchabfrage führt einen ODER-Vorgang mit den angegebenen Tags aus. Sie können jedoch die Option „Übereinstimmung mit allen Tags“ nutzen, um nach Assets zu suchen, die alle angegebenen Tags enthalten.
...
1. Klicken Sie in der Symbolleiste auf **[!UICONTROL Löschen]** ![deleteinnerhalb](assets/do-not-localize/deleteoutline.png).
1. Klicken Sie im Bestätigungsdialogfeld auf **[!UICONTROL Löschen]**, um die benutzerdefinierten Änderungen zu entfernen.

   Nachdem Sie die benutzerdefinierten Änderungen an den Suchfacetten gelöscht haben, wird das Schlosssymbol ![Schlosssymbol](assets/do-not-localize/lock_closed_icon.svg) vor **[!UICONTROL Assets Admin Search Rail]** auf der Seite **[!UICONTROL Forms suchen]** erneut angezeigt.
...
| ------------------- | ---------------------------------------------------------------- |
| [!UICONTROL Bearbeiten] | Lesen und Schreiben von Berechtigungen für den Knoten `/apps` in CRXDE |
| [!UICONTROL Löschen] | Berechtigungen zum Lesen, Schreiben und Löschen für den Knoten `/apps` in CRXDE |
| [!UICONTROL Vorschau] | Berechtigungen zum Lesen, Schreiben und Löschen auf dem `/var/dam/content`-Knoten in CRXDE. Außerdem Lese- und Schreibberechtigungen für den Knoten `/apps` |
