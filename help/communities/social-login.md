---
title: Social Login mit Facebook und Twitter
description: Mit der Social-Media-Anmeldung können sich Besucher der Website mit ihrem Facebook- oder Twitter-Konto anmelden.
...
ht-degree: 2%
---
# Social Login mit Facebook und Twitter {#social-login-with-facebook-and-twitter}
...
Um die Social-Media-Anmeldung einzuschließen *müssen* benutzerdefinierte Facebook- und Twitter-Anwendungen erstellen.

Während das Beispiel „we-retail“ Beispiele für Facebook- und Twitter-Anwendungen sowie Cloud-Services bietet, sind diese nicht auf einer [Produktions-Website](../../help/sites-administering/production-ready.md) verfügbar.
...
1. **Umfang** (Berechtigungen) gibt die Daten an, die die App anfordern darf.
   ...
   * Die Instanzen Facebook und Twitter [Adobe Granite OAuth Application and Provider](#adobe-granite-oauth-application-and-provider) enthalten standardmäßig die grundlegenden App-Berechtigungen in ihrem Gültigkeitsbereich.
...
Die soziale Anmeldung und das we-retail-Facebook-Beispiel wurden entwickelt, als die Facebook Graph API Version 1.0 war.
Ab AEM 6.4 GA und AEM 6.3 SP1 wurde die Social-Media-Anmeldung für die Verwendung mit der neueren Facebook Graph API 2.5-Version aktualisiert.
...
Eine ordnungsgemäß konfigurierte Facebook-Anwendung ist erforderlich, um die Anmeldung über Facebook zu ermöglichen.

Um eine Facebook-Anwendung zu erstellen, folgen Sie den Anweisungen von Facebook unter [https://developers.facebook.com/apps/](https://developers.facebook.com/apps/). Änderungen an ihren Anweisungen werden in den folgenden Informationen nicht angezeigt.

Im Allgemeinen gilt ab Facebook-API v2.7:

* *Neue Facebook-App hinzufügen*
  * Wählen Sie *Platform* die Option Website:
    * Geben Sie *Site-*) `  https://<server>:<port>.`
    * Geben *unter &quot;*&quot; einen Titel ein, der als Titel des Facebook Connect-Dienstes verwendet werden soll.
...
Die [Adobe Granite OAuth Application and Provider](#adobe-granite-oauth-application-and-provider)-Instanz, die durch Erstellen einer Cloud-Service-Konfiguration instanziiert wurde, identifiziert die Facebook-Anwendung und die Mitgliedsgruppen, zu denen die neuen Benutzer hinzugefügt werden.

1. Melden Sie sich auf der AEM-Autoreninstanz mit Administratorrechten an.
1. Wählen Sie in der globalen Navigation **[!UICONTROL Tools]** > **[!UICONTROL Cloud Services]** > **[!UICONTROL Konfiguration zur Anmeldung über Facebook]**.
...
Der AEM Communities-Anbieter erweitert die OAuth-Anwendungsinstanz und die Provider[&#128279;](#adobe-granite-oauth-application-and-provider)-Instanz von Adobe Granite.
...
Wenn eine Bearbeitung erforderlich ist, führen Sie auf jeder AEM-Veröffentlichungsinstanz folgende Schritte aus:
...
1. Wählen Sie das Stiftsymbol aus, um es zur Bearbeitung zu öffnen.
   ...
   * **[!UICONTROL Felder]**
     ...
     Wenn Felder aktiviert sind, werden beim Aufruf der Facebook-Graph-API die folgenden Felder einbezogen. Die Felder müssen innerhalb des in der Cloud Service-Konfiguration definierten Bereichs zulässig sein. Zusätzliche Felder bedürfen möglicherweise der Genehmigung durch Facebook. Weitere Informationen finden Sie im Abschnitt Facebook-Anmeldeberechtigungen in der Facebook-Dokumentation. Die Standardfelder werden als Parameter hinzugefügt:
...
Eine konfigurierte Twitter-Anwendung ist erforderlich, um die Anmeldung bei Twitter zu aktivieren.

Befolgen Sie die neuesten Anweisungen, um eine Twitter-Anwendung unter [https://apps.twitter.com](https://apps.twitter.com/) zu erstellen.

Allgemein:

1. Geben Sie *Name* ein, um Ihre Twitter-Anwendung für die Benutzer Ihrer Website zu identifizieren.
...
Im Bereich Berechtigungen der Twitter-Anwendungsverwaltung:
...
* **[!UICONTROL Zusätzliche Berechtigungen]**: Wählen Sie optional &quot;`Request email addresses from users`&quot;.
  ...
  * In der Anleitung auf Twitter werden zusätzliche Schritte vermerkt.

Die einzige REST-Anfrage für die Anmeldung über Social Media ist *[Konto abrufen/Anmeldeinformationen überprüfen](https://dev.twitter.com/rest/reference/get/account/verify_credentials)*.

### Erstellen einer Twitter Connect Cloud Service {#create-a-twitter-connect-cloud-service}

Die Instanz [Adobe Granite OAuth Application and Provider](#adobe-granite-oauth-application-and-provider), die durch Erstellen einer Cloud-Service-Konfiguration instanziiert wurde, identifiziert die Twitter-Anwendung und die Mitgliedsgruppen, zu denen die neuen Benutzer hinzugefügt werden.

1. Melden Sie sich auf der Autoreninstanz mit Administratorrechten an.
1. Wählen Sie in der globalen Navigation **[!UICONTROL Tools]** > **[!UICONTROL Cloud Services]** > **[!UICONTROL Konfiguration zur Anmeldung über Twitter]**.
1. Wählen Sie die Konfiguration **[!UICONTROL Kontextpfad]** .
...
1. Erstellen/Bearbeiten Sie die Twitter-Cloud-Service-Konfiguration.
   ...
   * **[!UICONTROL Benutzer erstellen]**
     ...
     Wenn diese Option aktiviert ist, wird bei der Anmeldung mit einem Twitter-Konto ein AEM-Benutzereintrag erstellt und als Mitglied zu den ausgewählten Benutzergruppen hinzugefügt. Standard ist aktiviert (wird dringend empfohlen).
...
Die AEM Communities-Konfiguration erweitert die Instanz [Adobe Granite OAuth Application and Provider](#adobe-granite-oauth-application-and-provider). Dieser Anbieter muss bearbeitet werden, damit Benutzeraktualisierungen zulässig sind.
...
1. Wählen Sie das Stiftsymbol aus, um es zur Bearbeitung zu öffnen.
   ...
   * **[!UICONTROL Cloud Service-Konfiguration]**
     ...
     Der Standardwert lautet *conf.* Nicht bearbeiten.
...
* Melden Sie sich mit den entsprechenden Anmeldeinformationen an, falls Sie noch nicht bei Facebook oder Twitter angemeldet sind.
* Je nach dem von der Facebook- oder Twitter-App angezeigten Dialogfeld kann es erforderlich sein, eine Berechtigung zu erteilen.
* Beachten Sie, dass die Symbolleiste am oberen Seitenrand aktualisiert wird, um die erfolgreiche Anmeldung widerzuspiegeln.
* Wählen Sie **[!UICONTROL Profil]** aus: Auf der Seite Profil werden das Avatarbild, der Vorname und der Nachname des Benutzers angezeigt. Es zeigt auch die Informationen aus dem Facebook- oder Twitter-Profil entsprechend den zulässigen Feldern/Parametern an.
...
>[!CAUTION]
>
>Achten Sie darauf, den Authentifizierungs-Handler nicht mit einer Facebook- oder Twitter-Instanz von *Adobe Granite OAuth Application and Provider* zu verwechseln.
...
1. Suchen Sie Adobe Granite OAuth Application and Provider.
   ...
   * **[!UICONTROL Callback-URL]**
   ...
   >[!NOTE]
   >
   >Die Domain für den Callback muss beim Provider (Facebook oder Twitter) registriert sein.
...
Bei Community-Websites, auf denen sich Hunderttausende von Nutzern mit ihrer Facebook- oder Twitter-Anmeldung registrieren, kann die Durchlaufleistung der Abfrage, die durchgeführt wird, wenn ein Site-Besucher seine Social-Media-Anmeldung verwendet, durch Hinzufügen des folgenden Oak-Index verbessert werden.
...
* Ersetzen Sie für **name**-`oauthid-123` *123* durch den Facebook ***App ID***- oder Twitter ***Consumer (API)-Schlüssel***. Dies ist der Wert der **Client ID** in der Konfiguration für die [Adobe Granite OAuth Application and Provider](social-login.md#adobe-granite-oauth-application-and-provider).
