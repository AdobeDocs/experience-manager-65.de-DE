...
## Schnellstart (SOAP-Modus): Rendern eines interaktiven PDF-Formulars mithilfe der Java™-API {#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api}
...
## Schnellstart (SOAP-Modus): Rendern eines Formulars auf dem Client mithilfe der Java™-API {#quick-start-soap-mode-rendering-a-form-at-the-client-using-the-java-api}

Im folgenden Code-Beispiel wird ein Formular namens *Loan.xdp* mithilfe der Forms Service-Java™-API auf dem Client gerendert. Beachten Sie, dass der Formularentwurf Teil eines Programms ist und durch Verwendung des Inhaltsstamm-URI-Werts `repository:///` referenziert wird. (Siehe [Rendern von Formularen auf dem Client](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client).)
...
## Schnellstart (SOAP-Modus): Rendern eines Handbuchs (veraltet) mithilfe der Java™-API {#quick-start-soap-mode-rendering-a-guide-deprecated-using-the-java-api}
...
## Schnellstart (SOAP-Modus): Rendern eines Formulars, das auf Fragmenten basiert, mithilfe der Java™-API {#quick-start-soap-mode-rendering-a-form-based-on-fragments-using-the-java-api}
...
## Schnellstart (SOAP-Modus): Rendern eines HTML-Formulars mithilfe der Java™-API {#quick-start-soap-mode-rendering-an-html-form-using-the-java-api}

Im folgenden Code-Beispiel wird ein HTML-Formular mithilfe der Forms Service-Java™-API gerendert. Dem HTML-Formular werden eine Symbolleiste und zwei Dateianlagen hinzugefügt. Darüber hinaus wird der Wert des Benutzeragenten vom `HttpServletRequest`-Objekt abgerufen. (Siehe [Wiedergeben von Formularen als HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html).)
...
Im folgenden Code-Beispiel wird ein HTML-Formular verarbeitet, das als XML-Daten gesendet wird. Der Wert für den Content-Typ, der an die Methode `processFormSubmission` übergeben wird, ist `CONTENT_TYPE=application/x-www-form-urlencoded`. Die Werte, die den Feldern mit den Namen `mortgageAmount`, `lastName` und `firstName` entsprechen, werden angezeigt. Eine benutzerdefinierte Methode mit dem Namen `getNodeText` wird in dieser Schnellstartanleitung verwendet. Sie akzeptiert eine `org.w3c.dom.Document`-Instanz und einen Zeichenfolgenwert, der den Knotennamen angibt. Diese Methode gibt einen Zeichenfolgenwert zurück, der den Wert des Knotens darstellt. (Siehe [Umgang mit gesendeten Formularen](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
## Schnellstart (SOAP-Modus): Vorausfüllen von Formularen mit flexiblen Layouts mithilfe der Java™-API {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}
...
* `renderPOForm`: Verwendet die Java™-API des Forms-Dienstes zum Rendern eines dynamischen Bestellformulars. Das `com.adobe.idp.Document`-Objekt, das von der Methode `convertDataSource` zurückgegeben wurde, wird verwendet, um das Formular vorauszufüllen.

  Alle diese Methoden werden innerhalb der Methode `doPost` des Java™-Servlets aufgerufen. (Siehe [Vorausfüllen von Formularen mit fließfähigen Layouts](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts).)
...
## Schnellstart (SOAP-Modus): Verarbeiten eines Formulars, das ein Berechnungsskript enthält, mithilfe der Java™-API {#quick-start-soap-mode-handling-a-form-containing-a-calculation-script-using-the-java-api}
...
## Schnellstart (SOAP-Modus): Rendern nach Wert mithilfe der Java™-API {#quick-start-soap-mode-rendering-by-value-using-the-java-api}

Der folgende Java™-Schnellstart rendert ein interaktives PDF-Formular, das auf einem Formularentwurf mit dem Namen *Loan.xdp* basiert, nach Wert. Beachten Sie, dass der Formularentwurf zum Ausfüllen eines `com.adobe.idp.Document`-Objekts namens *inputXDP* verwendet wird. (Siehe [Rendern von Formularen nach Wert](/help/forms/developing/rendering-forms.md#rendering-forms-by-value).)
