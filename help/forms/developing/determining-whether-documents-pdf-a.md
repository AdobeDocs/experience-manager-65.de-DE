...
Es muss auf ein DDX-Dokument verwiesen werden, um einen Assembler-Dienst-Vorgang auszuführen. Um festzustellen, ob ein PDF-Eingabedokument PDF/A-kompatibel ist, stellen Sie sicher, dass das DDX-Dokument das `PDFAValidation`-Element in einem `DocumentInformation`-Element enthält. Das `PDFAValidation`-Element weist den Assembler-Dienst an, ein XML-Dokument zurückzugeben, das angibt, ob das PDF-Eingabedokument PDF/A-kompatibel ist.
...
Nachdem Sie den Assembler-Dienst-Client erstellt haben, auf das DDX-Dokument verwiesen haben, auf ein interaktives PDF-Dokument verwiesen haben und Laufzeitoptionen festgelegt haben, können Sie den `invokeDDX`-Vorgang aufrufen. Da das DDX-Dokument das `DocumentInformation`-Element enthält, gibt der Assembler-Dienst XML-Daten anstelle eines PDF-Dokuments zurück.
...
So ermitteln Sie mithilfe der Assembler-Dienst-API (Java), ob ein PDF-Dokument PDF/A-kompatibel ist:
...
1. Referenzieren Sie ein vorhandenes DDX-Dokument.
   ...
   * Erstellen Sie ein `java.io.FileInputStream`-Objekt, das das DDX-Dokument darstellt, indem Sie seinen Konstruktor verwenden und einen Zeichenfolgenwert übergeben, der den Speicherort der DDX-Datei angibt. Um festzustellen, ob das PDF-Dokument PDF/A-kompatibel ist, stellen Sie sicher, dass das DDX-Dokument das `PDFAValidation`-Element enthält, das in einem `DocumentInformation`-Element enthalten ist.
...
Stellen Sie mithilfe der Assembler-Dienst-API (Webservice) fest, ob ein PDF-Dokument PDF/A-kompatibel ist:
...
1. Rufen Sie Informationen zum PDF-Dokument ab.
   ...
   Rufen Sie die Methode `invoke` des `AssemblerServiceService`-Objekts auf und übergeben Sie die folgenden Werte:
