...
Bei der Betrachtung wird der Venia Reference store](https://github.com/adobe/aem-cif-guides-venia) verwendet. [ Beachten Sie, dass der Komponenten-Proxy-Namen `venia/components/structure/navigation` und **nicht** der Name der CIF-Navigationskomponente (`core/cif/components/structure/navigation/v1/navigation`) verwendet wird.
...
>[!NOTE]
>
>Während serverseitige Daten vom AEM Dispatcher zwischengespeichert werden können, holen einige CIF-Komponenten wie `product`-, `productlist`- und `searchresults`-Komponenten in der Regel immer die Produktpreise in einer clientseitigen Browseranforderung erneut ab, wenn die Seite geladen wird. Dadurch wird sichergestellt, dass beim Laden der Seite immer die wesentlichen dynamischen Inhalte abgerufen werden.
