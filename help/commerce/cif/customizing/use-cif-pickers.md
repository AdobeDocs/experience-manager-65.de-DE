...
- selectionId (id, uid, sku, slug, CombinedSlug, CombinedSku) - ermöglicht die Auswahl des Produktattributs, das vom Picker zurückgegeben werden soll (Standard = id). Bei Verwendung von „sku“ wird die SKU des ausgewählten Produkts zurückgegeben, wenn „combinedSku“ verwendet wird. Zudem wird eine Zeichenfolge wie „base#variant“ mit den SKUs des Basisprodukts und der ausgewählten Variante oder eine einzelne SKU zurückgegeben, wenn ein Basisprodukt ausgewählt ist.
...
Außerdem können Standardeigenschaften für Dialogfelder wie `name`, `fieldLabel`oder `fieldDescription`, werden ebenfalls unterstützt.
