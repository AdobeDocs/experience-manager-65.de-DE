---
cloud: Experience Cloud
product: adobe experience manager
audience: end-user
user-guide-title: AEM 6.5-Mobilbenutzeranleitung
breadcrumb-title: Mobilanleitung
user-guide-description: Erstellen und verwalten Sie Inhalte für Mobile Apps.
feature: Mobile
role: User
hide: true
source-git-commit: bca6156727dca11b2e09be549f3def6130827193
workflow-type: tm+mt
source-wordcount: '329'
ht-degree: 13%

---


# AEM 6.5-Mobilbenutzeranleitung {#mobile}

+ [Benutzerhandbuch für Mobilgeräte - Übersicht](home.md)
+ Einführung in AEM Mobile {#introduction}
   + [Erste Schritte mit AEM Mobile](getting-started-aem-mobile.md)
   + [AEM Mobile On-Demand](getting-started-aem-mobile-on-demand.md)
   + [Mobile Apps](mobile-apps.md)
   + [AEM Adobe PhoneGap](getting-started-aem-mobile-phonegap.md)
   + [AEM Mobile - Einhaltung der DSGVO](aem-mobile-gdpr-compliance.md)
   + [Mobile Apps entwickeln](develop-mobile-apps.md)
+ Erstellen der On-Demand-Services-App {#authoring}
   + [Mobile On-Demand](mobile-apps-ondemand.md)
   + [Verwalten von Inhalt](mobile-apps-ondemand-manage-content-ondemand.md)
   + [Veröffentlichen von Inhalt aufheben der Veröffentlichung](mobile-on-demand-publishing-unpublishing.md)
   + [AEM Mobile-Anwendungs-Dashboard](mobile-apps-ondemand-application-dashboard.md)
   + [Anwendungserstellungs- und Konfigurationsaktionen](mobile-apps-ondemand-application-create-configure-action.md)
   + [Artikel verwalten](mobile-on-demand-managing-articles.md)
   + [Hochladen freigegebener Ressourcen](mobile-on-demand-shared-resources.md)
   + [Cloud-Konfiguration](mobile-on-demand-associating-an-on-demand-app-to-cloud-configuration.md)
   + [Verwalten von Bannern](mobile-on-demand-managing-banners.md)
+ Entwickeln einer On-Demand-Service-App {#developing}
   + [AEM Mobile On-Demand](aem-mobile-on-demand.md)
   + [Programmvorlagen und -komponenten](app-templates-and-components1.md)
   + [Inhaltseigenschaften und Knoten](content-properties.md)
   + [Erstellen der Exportkonfiguration für freigegebene Ressourcen](creating-shared-resources-export-configuration.md)
   + [Erstellen und Hinzufügen von Vorlagen und Komponenten](mobile-ondemand-app-templates.md)
   + [Best Practices](best-practices-aem-mobile.md)
   + [Erstellen der Konfiguration für den Artikelexport](creating-article-export-configuration.md)
   + [Mobile mit Inhaltssynchronisierung](mobile-ondemand-contentsync.md)
   + [Seitenvorlagen für Mobile Apps](apps-architecture.md)
   + [Exportieren von Inhalten mit Inhaltseigenschaften](on-demand-content-properties-exporting.md)
   + [Rendering und Versand](rendering-and-delivery.md)
   + [Content Services](developing-content-services.md)
   + [Inhaltsbereitstellung](develop-content-as-a-service.md)
+ Administration der On-Demand-Services-App {#administering}
   + [AEM Mobile On-Demand](aem-mobile.md)
   + [AEM Mobile-Setup](aem-mobile-setup.md)
   + [Konfigurieren von Benutzern und Benutzergruppen](aem-mobile-configure-users.md)
   + [Vorschau mit Preflight](aem-mobile-manage-ondemand-services.md)
   + [Mobile Apps](administer-mobile-apps.md)
   + [Modelle - Übersicht](model-management.md)
   + [Leerzeichen und Entitäten](spaces-and-entities.md)
   + [Verwalten von Sammlungen](mobile-on-demand-managing-collections.md)
+ Authoring mit Adobe PhoneGap Enterprise {#author-phonegap}
   + [PhoneGap](phonegap.md)
   + [Bearbeiten von App-Metadaten](phonegap-editmetadata.md)
   + [App-Definitionen](phonegap-app-definitions.md)
   + [Erstellen und Verwalten von App-Inhalten](phonegap-manage-app-content.md)
   + [Erstellen einer AEM Mobile-App mit dem Assistenten „Erstellen“](phonegap-create-new-app.md)
   + [Erstellen von Mobile Apps](phonegap-authoring-apps.md)
   + [Programmkachel verwalten](phonegap-app-details-tile.md)
   + [AEM Mobile-Inhaltspersonalisierung](phonegap-aem-mobile-content-personalization.md)
+ Entwickeln von Adobe PhoneGap Enterprise {#developphonegap}
   + [PhoneGap](developing-in-phonegap.md)
   + [Erstellen von Mobile Apps](building-app-mobile-phonegap.md)
   + [Push-Benachrichtigungen](phonegap-push-notifications.md)
   + [Entwickeln von Mobile Apps in AEM](developing-mobile-applications.md)
   + [Mobile mit Inhaltssynchronisierung](phonegap-contentsync.md)
   + [Die Anatomie einer App](phonegap-apps-arch.md)
   + [Entwickeln von Apps mit PhoneGap CLI](phonegap-apps-pg-cli.md)
   + [Vorkonfigurierte App-Handler](contentsync-app-handlers.md)
   + [Erstellen und Bearbeiten von Apps mit der Apps-Konsole](phonegap-apps-console.md)
   + [Ist Ihre Hybrid-App bereit für AEM Mobile?](phonegap-adding-content-to-imported-app.md)
   + [Zugriff auf Gerätefunktionen](phonegap-access-device-features.md)
   + [Adobe Analytics zu Ihrer Mobile App hinzufügen](phonegap-add-analytics-to-apps.md)
   + [Seitenvorlagen für Mobile Apps](phonegap-apps-arch-page-templates.md)
   + [Konfigurieren von Adobe Target Cloud Service](aem-mobile-configuring-cloud-service.md)
   + [Single Page Applications (SPA)](phonegap-single-page-applications.md)
   + [Starten von AEM PhoneGap](starting-aem-phonegap-app.md)
   + [Mobile Apps testen](develop-mobile-apps-testing.md)
   + [Strukturieren einer App](phonegap-structure-an-app.md)
   + [App-Leistung mit Adobe Mobile Analytics verfolgen](phonegap-intro-to-app-analytics.md)
+ Verwaltung von Adobe PhoneGap Enterprise {#administering-phonegap}
   + [Schnellstart für Mobilgeräte](phonegap-mobile-quickstart.md)
   + [Verwalten von Inhalten für Adobe PhoneGap Enterprise mit AEM](administer-phonegap.md)
   + [Konfigurieren von Adobe PhoneGap Build Cloud Service](configure-phonegap-build-cloud.md)
   + [Festlegen des Referrer-Filters auf „Leer zulassen“](setting-referrer-filter-empty.md)
   + [Konfigurieren der Adobe Mobile Services-Cloud Service](configure-adobe-mobile-cloud-service.md)
   + [Konfigurieren von Benutzern und Benutzergruppen](configure-users-groups.md)
