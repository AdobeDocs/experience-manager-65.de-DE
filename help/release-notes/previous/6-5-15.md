...
* Wenn Sie den Farbkontrast-Analysator im Experience Manager verwenden, gibt es in der Datumsauswahl des Kalender-Widgets keine farbliche Unterscheidung zwischen dem aktuellen Datum und dem gewählten Datum. Es liegt kein Kontrastverhältnis von mindestens 3:1 im Vergleich zu den angrenzenden Farben vor. (ASSETS-6911)
...
* Für Platzhaltertexte, Pfad und alternativen Text liegt kein Kontrastverhältnis von mindestens 4.5:1 im Vergleich zu ihrer Hintergrundfarbe vor. (ASSETS-6884, ASSETS-6865)
...
* Für den Text des Ordnernamens auf der Hauptnavigationsseite liegt kein Kontrastverhältnis von 4.5:1 im Vergleich zur Hintergrundfarbe vor. (ASSETS-6872)
...
* Für die Umfangs-, Workflow- und Zeitzonenoptionen liegt kein Kontrastverhältnis von 4.5:1 im Vergleich zur Hintergrundfarbe vor. (ASSETS-6868)
...
* Für die auf der Registerkarte „IPTC“ der Asset-Eigenschaften verfügbaren leeren Texteingabefelder liegt kein Kontrastverhältnis von 3:1 im Vergleich zu den angrenzenden Farben vor. (ASSETS-6854, ASSETS-6847)
...
* Wenn `=` Zeichen in den Kontrollkästchen, Dropdown-Listen oder Optionsfeldern eines adaptiven Formulars verwendet wird und das Datensatzdokument generiert wird, ist `=` Zeichen im generierten Datensatzdokument nicht sichtbar. (NPR-38859)
* Nach der Aktualisierung auf Service Pack 6.5.11.0 ist die Anzahl der Fehler bei der Stapelverarbeitung von Hinweisen um ein Vielfaches gestiegen. (NPR-39636)
...
* Wenn Benutzende das mit IBM® WebSphere® bereitgestellte AEM Forms Service Pack 14 (SP14) von AEM Forms anwenden, schlägt das Bootstrapping bei der Initialisierung einer Datenbank fehl und der `java.lang.NoClassDefFoundError:org/apache/log4j/Logger` Fehler tritt auf. (NPR-39414)
...
* Wenn ein(e) Benutzende(r) versucht, die Connector-Konfiguration mithilfe von Configuration Manager zu aktualisieren, schlägt dies fehl.(CQ-4347077)
...
* Für den Textinhalt liegt kein Kontrastverhältnis von 4.5:1 vor. (SITES-8756, SITES-8885)
* Für den Text von Links oder Schaltflächen liegt kein Kontrastverhältnis von 4.5:1 vor, wenn der Mauszeiger über diese bewegt oder darauf fokussiert wird. (SITES-8756, SITES-8885)
...
* Das Multifield in einem „Inhaltsfragment-Modell“ kann nicht geleert und gespeichert werden, da die Validierung auch dann erfolgt, wenn **[!UICONTROL Erforderlich]** nicht ausgewählt ist. (NPR-39063)
...
* [!DNL Experience Manager] 6.5.15.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden [ in ](/help/sites-deploying/upgrade.md)Upgrade-Dokumentation“. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Das OSGi-Bundle `org.apache.jackrabbit.oak-core` Version 1.22.13 oder höher (Zu verwendende Web-Konsole: `/system/console/bundles`).
...
Dieses Paket ist für Kundinnen und Kunden erforderlich, die GraphQL verwenden. Dadurch können sie basierend auf den tatsächlich verwendeten Funktionen die erforderliche Indexdefinition hinzufügen.
...
* AEM Forms JEE 6.5.15.0 Service Pack wurde nicht ordnungsgemäß in der JBoss Linux-Umgebung installiert. Überprüfen Sie die ordnungsgemäße Installation des AEM Forms JEE 6.5.15.0 Service Pack, indem Sie in der Datei `PatchInstallerProcessing[1-9*].log` nach dem Protokolleintrag `[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component is not in the installation. Skipping Processing` suchen. Dieser Eintrag zeigt an, dass die Installation des AEM Forms JEE 6.5.15.0 Service Pack nicht erfolgreich war. Um das Installationsproblem mit dem AEM Forms JEE 6.5.15.0 Service Pack zu beheben, [fügen Sie die Datei „RUP_BOM.xml“](/help/forms/using/linux-jboss-installation-issue-on-sp15.md) manuell zum XML-Verzeichnis hinzu.

* Durch die Installation des Service Pack wird die Spracheigenschaft des Knotens „/content/cq:tags“ auf den Standardwert zurückgesetzt. Daher ist es erforderlich, sie vor der Installation aus den Eigenschaften hinzuzufügen.
