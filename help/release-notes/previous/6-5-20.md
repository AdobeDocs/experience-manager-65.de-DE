...
* Der Status `scene7SmartCropProcessingStatus` wurde korrigiert. Beim intelligenten Zuschnitt von Videometadaten wurde ein Fehler angezeigt, selbst wenn der Vorgag erfolgreich war. (ASSETS-31255)
...
* Wenn Benutzende das Datumsfeld mit Anzeigemuster in AEM Form Service Pack 6.5.18.0 auswählen, können sie das aktuelle Datum nicht über die Tastatur auswählen. (FORMS-12736)
...
* Ein Autor verwendet die „Adaptive Forms - Einbettungskomponente (v2)“, um ein adaptives Formular in seine Sites-Seite einzubetten. Wenn das eingebettete Formular eine CAPTCHA-Komponente enthält (CAPTCHA-Dienst > reCAPTCHA, Einstellungen > reCAPTCHA-v2), wird die Siteseite nicht gerendert. Dies tritt auf, wenn der/die Benutzende versucht, die Sites-Seite mit „Als veröffentlicht anzeigen“ in der Autoreninstanz anzuzeigen. Der folgende Fehler wird als angezeigt (FORMS-11859):
...
* Wenn Benutzende in AEM Forms Service Pack 6.5.18.0 einen „Empfangsvorgang“ für „EmailService“ aufrufen, wird folgende Ausnahme ausgelöst (FORMS-12050):
  `java.util.ServiceConfigurationError: javax.mail.Provider: Provider com.sun.mail.imap.IMAPProvider not a subtype`

* Wenn der FIPS-Modus in AEM Forms Service Pack 6.5.18.0 aktiviert ist, schlägt das Erstellen einer Benutzerin bzw. eines Benutzers unter Standard-DOM mit folgendem Fehler fehl (FORMS-11857):
...
* Benutzende können in AEM Forms Service Pack 6.5.18.0 keine Sicherheitseinstellungen erstellen. Es werden keine Fehler- oder Server-Protokolle angezeigt, dafür aber eine Popup-Fehlermeldung am Bildschirm. (FORMS-12212)

* Wenn Benutzende in AEM Forms Service Pack 6.5.18.0 ein adaptives Formular im JEE-Workflow absenden, wird der Anhang des adaptiven Formulars nicht an den JEE-Prozess gesendet, was zu einem Anwendungsfehler führt. (FORMS-12232, FORMS-12228)
...
* Wenn Benutzende beim Ausführen des Konfigurations-Managers in AEM Forms 6.5.19.1 mit der JBoss® Turnkey-Einrichtung die Option „Microsoft® SharePoint“ auswählen, schlägt die Installation von LiveCycle JBoss® EAR fehl und es wird der folgende Fehler angezeigt: (FORMS-12463)
...
* Wenn Benutzende ein Dokumentfragment mithilfe des Formulardatenmodells in AEM Forms Service Pack 6.5.19.0 erstellen, werden die Variablennamen im seitlichen Panel undefiniert angezeigt. Die Variablennamen werden jedoch korrekt angezeigt, wenn sie im Formularbereich abgelegt werden oder wenn auf sie geklickt wird. (FORMS-13238)


#### [!DNL Forms Designer] {#forms-designer-6520}


* Wenn Benutzende auf AEM Forms Service Pack 6.5.18.0 aktualisieren, schlagen aufgrund fehlender Ausnahmebehandlung XDPs fehl, die über den Output-Dienst mit aktivierter Option „PDF mit Tags“ übergeben werden. (LC-3921757)
...
* [!DNL Experience Manager] 6.5.20.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden [&#x200B; in &#x200B;](/help/sites-deploying/upgrade.md)Upgrade-Dokumentation“. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* [Liste der in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-packages.zip) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
