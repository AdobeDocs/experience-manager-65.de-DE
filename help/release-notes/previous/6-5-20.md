...
* Wenn ein(e) Benutzende(r) das Datumsfeld auswählt, das in AEM Forms Service Pack 6.5.18.0 ein Anzeigemuster aufweist, kann der/die Benutzende das aktuelle Datum nicht über die Tastatur auswählen. (FORMS-12736)
* Wenn auf der AEM Forms Service Pack 6.5.17.0- und Service Pack 6.5.18.0 im Kalender-Widget zwischen Monaten gewechselt wird, zeigt die Datumsauswahl-Komponente eine zusätzliche Zeile an. (FORMS-11869)
...
* Wenn ein Benutzer auf AEM Form Service Pack 6.5.19.0 aktualisiert, wird nach dem Aktualisieren einer neuen Sprache auf das vorhandene Wörterbuch diese nicht mit den „guideContainer“-Zeilen zusammengeführt, um einem Formular ein Gebietsschema hinzuzufügen. (FORMS-12947)

* Auf AEM Forms Service Pack 6.5.19.0 schlägt der aufgerufene Webservice-Vorgang auf Java™ 11 mit dem Fehler fehl (FORMS-12329):
  `java.lang.NoClassDefFoundError message:sun/misc/BASE64Decoder`

* Wenn ein(e) Benutzende(r) den Vorgang „Empfangen“ für „EmailService“ auf AEM Forms Service Pack 6.5.18.0 aufruft, wird eine Ausnahme ausgelöst (FORMS-12050):
  `java.util.ServiceConfigurationError: javax.mail.Provider: Provider com.sun.mail.imap.IMAPProvider not a subtype`

* Wenn der FIPS-Modus auf AEM Forms Service Pack 6.5.18.0 aktiviert ist, schlägt das Erstellen eines Benutzers unter dem Standard-DOM mit der folgenden Fehlermeldung fehl (FORMS-11857):
...
* Auf AEM Forms Service Pack 6.5.18.0 kann der/die Benutzende keine Sicherheitseinstellungen erstellen, es werden keine Fehler- oder Serverprotokolle angezeigt, es wird jedoch eine Popup-Fehlermeldung auf dem Bildschirm angezeigt. (FORMS-12212)

* Wenn ein(e) Benutzende(r) in AEM Forms Service Pack 6.5.18.0 ein adaptives Formular über den JEE-Workflow sendet, wird die Anlage im adaptiven Formular nicht an den JEE-Prozess gesendet, was zu einem Anwendungsfehler führt. (FORMS-12232, FORMS-12228)
...
* Wenn ein adaptives Formular in AEM Forms 6.5.18.0 veröffentlicht wird, werden alle seine Abhängigkeiten, einschließlich der Richtlinien, erneut veröffentlicht, auch wenn keine Änderungen daran vorgenommen wurden. (FORMS-10454)

* Wenn ein(e) Benutzende(r) beim Ausführen des Konfigurations-Managers auf AEM Forms 6.5.19.1 mit JBoss® Turnkey-Setup &quot;Microsoft® SharePoint&quot; auswählt, schlägt die LiveCycle JBoss® EAR-Installation fehl und die folgende Fehlermeldung wird angezeigt: (FORMS-12463)
...
* Wenn ein(e) Benutzende(r) ein Dokumentfragment mithilfe des Formulardatenmodells in AEM Forms Service Pack 6.5.19.0 erstellt, werden die Variablennamen im Seitenbereich nicht definiert angezeigt. Die Variablennamen werden jedoch korrekt angezeigt, wenn sie im Formularbereich abgelegt werden oder wenn auf sie geklickt wird. (FORMS-13238)


#### [!DNL Forms Designer] {#forms-designer-6520}


* Wenn Benutzende ein Upgrade auf AEM Forms Service Pack 6.5.18.0 durchführen, schlagen XDPs, die über den Ausgabe-Service mit aktivierter PDF-Option mit Tags übergeben werden, aufgrund fehlender Ausnahmebehandlung fehl. (LC-3921757)
...
* [!DNL Experience Manager] 6.5.20.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden Sie in der [Dokumentation zu Upgrades](/help/sites-deploying/upgrade.md). <!-- UPDATE FOR EACH NEW RELEASE -->
...
* [Liste der in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.20.0](/help/release-notes/assets/65200-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
