...
* Die `htmlwriter` kodiert die Attributwerte nicht. Das Markup im XF-Markup wird mit dekodierten Attributwerten exportiert (d. h. `"` anstelle von `&#34`). Es verursacht auf der Target-Seite Probleme mit Visual Experience Composer, der das exportierte XF-Markup verwendet (NPR-34048).
...
* Erstellen einer Marke unter `/content/campaign` führt zu einer Struktur, die nicht das Erstellen von Kampagnen zulässt. Für eine mit der Option [!UICONTROL Marke erstellen] erstellte Marke ist es nicht möglich, [!UICONTROL Angebote und Aktivitäten] zu erstellen, da keine [!UICONTROL Erstellen] vorhanden ist (NPR-34113).
