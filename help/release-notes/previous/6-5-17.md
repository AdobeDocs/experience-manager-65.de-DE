...
[!DNL Experience Manager] 6.5.17.0 enthält neue Funktionen, wichtige von Kundschaft angeforderte Verbesserungen, Fehlerkorrekturen sowie Leistungs-, Stabilitäts- und Sicherheitsverbesserungen, die seit der ersten Verfügbarkeit von 6.5 im April 2019 veröffentlicht wurden. [Installieren Sie dieses Service Pack](#install) auf [!DNL Experience Manager] 6.5.
...
* **DASH-Streaming mit Dynamic Media** - Unterstützung des neuen Protokolls (DASH - Dynamic Adaptive Streaming über HTTP) wurde für adaptives Streaming in der Dynamic Media-Videobereitstellung (mit aktiviertem CMAF) eingeführt.
...
* Nach der Aktualisierung auf das AEM 6.5.15.0 Service Pack funktionieren die HTML5-Formulare im Edge-Browser mit dem IE-Kompatibilitätsmodus nicht oder werden nicht ordnungsgemäß geladen. (FORMS-8526, FORMS-8523)
...
* Wenn Benutzende ein Upgrade auf das Service Pack von Experience Manager 6.5.16.0 durchführen, fehlen die CRX-Metadateneigenschaften nach der Übermittlung. (FORMS-8205)
...
* Wenn in Experience Manager 6.5.16.0 Forms Service Pack ein(e) Benutzende(r) versucht, die Richtliniensatzkoordinatoren zu bearbeiten, bleibt der Publisher des Managerdokuments immer deaktiviert. (FORMS-7775, FORMS-8599)
* Wenn Benutzende ein Upgrade auf das Service Pack von Experience Manager 6.5.16.0 durchführen, funktioniert die Methode „GuideNode.externalize“, die zu übersetzende Zeichenfolgen verarbeitet, nicht mehr. (FORMS-7709)
...
* Um die Leistung der PDFG-Konvertierung auf einem Experience Manager 6.5.16.0 Service Pack zu verbessern, wird das Ruheintervall konfiguriert. (FORMS-6752)
...
* Wenn Benutzende ein Upgrade auf das Service Pack von Experience Manager 6.5.15.0 durchführen, funktioniert die Umleitung nicht mehr, wenn ein adaptives Formular in Internet Explorer gerendert wird. (FORMS-6725)
...
* Wenn Benutzende auf einem JBoss® Turnkey-Server ein Upgrade auf das Service Pack von Experience Manager 6.5.16.0 durchführen, kann der Signature-Service nicht aufgerufen werden. Der aufgetretene Fehler lautet: `java.lang.ClassCastException: com.adobe.xfa.TextNode cannot be cast to com.adobe.xfa.Element`. (FORMS-7833)
* Nach dem Upgrade auf das Service Pack von Experience Manager 6.5.14.0 funktioniert Workbench nicht, wenn ein CRX-Knoten von einem Speicherort an einen anderen verschoben wird. Der Fehler wird als `ALC-CRX-30000-000: com.adobe.ep.crx.client.exceptions.CRCException: ALC-CRX-030-000-[Internal Server Error]` angezeigt. (FORMS-7713)
* Wenn ein(e) Benutzende(r) auf Experience Manager 6.5.16.0 Service Pack aktualisiert, kann der `Usage Rights` nicht angewendet werden. (FORMS-7892)
...
* Wenn ein(e) Benutzende(r) auf Experience Manager Forms 6.5.16.0 Service Pack aktualisiert und versucht, die `LicenseType` in `Production` zu ändern, werden die Änderungen nicht übernommen. (FORMS-7594)
...
* [!DNL Experience Manager] 6.5.17.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden [ in ](/help/sites-deploying/upgrade.md)Upgrade-Dokumentation“. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Bei einer Bereitstellung mit MongoDB und mehreren Instanzen installieren Sie [!DNL Experience Manager] 6.5.17.0 mit dem Package Manager auf einer der Autoreninstanzen.<!-- UPDATE FOR EACH NEW RELEASE -->
...
Das UberJar für [!DNL Experience Manager] 6.5.17.0 ist im [Maven Central Repository](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.17/) verfügbar. <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* Auf der JBoss® 7.1.4-Plattform schlägt die Bereitstellung von `adobe-livecycle-jboss.ear` bei der Installation des Service Packs von Experience Manager 6.5.16.0 oder höher fehl.
...
* Wenn der Brief in der Druckvorschau angezeigt wird, ändert sich der Inhalt. Einige Leerzeichen werden ausgeblendet und bestimmte Buchstaben werden durch „x“ ersetzt. (FORMS-15681)

## Enthaltene OSGi-Bundles und Inhaltspakete{#osgi-bundles-and-content-packages-included}

In den folgenden Textdokumenten sind die in [!DNL Experience Manager] 6.5.17.0 enthaltenen OSGi-Bundles und Inhaltspakete aufgeführt: <!-- UPDATE FOR EACH NEW RELEASE -->

* [Liste der in Experience Manager 6.5.17.0](/help/release-notes/assets/65170_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.17.0](/help/release-notes/assets/65170_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
