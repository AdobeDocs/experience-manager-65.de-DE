...
[!DNL Experience Manager] 6.5.19.0 enthält neue Funktionen, wichtige von Kundschaft angeforderte Verbesserungen, Fehlerkorrekturen sowie Leistungs-, Stabilitäts- und Sicherheitsverbesserungen, die seit der ersten Verfügbarkeit von 6.5 im April 2019 veröffentlicht wurden. [Installieren Sie dieses Service Pack](#install) auf [!DNL Experience Manager] 6.5.
...
* Wenn ein(e) Benutzende(r) versucht, PDF-Dokumente nach der Installation des Forms-Add-on-Pakets auf AEM Forms Service Pack 6.5.15.0 zu generieren, schlägt dies gelegentlich fehl und zeigt den Fehler an:
...
* [!DNL Experience Manager] 6.5.19.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden Sie in der [Dokumentation zu Upgrades](/help/sites-deploying/upgrade.md). <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Bei einer Bereitstellung mit MongoDB und mehreren Instanzen installieren Sie [!DNL Experience Manager] 6.5.19.0 mit dem Package Manager auf einer der Autoreninstanzen.<!-- UPDATE FOR EACH NEW RELEASE -->
...
* Auf der JBoss® 7.1.4-Plattform schlägt die Bereitstellung von `adobe-livecycle-jboss.ear` bei der Installation des Service Packs von Experience Manager 6.5.16.0 oder höher fehl. (CQ-4351522, CQDOC-20159)
...
#### Installieren des Servlet-Fragments (AEM Service Pack 6.5.14.0 oder früher)

* Wenn Sie ein Upgrade auf AEM Service Pack 6.5.15.0 oder höher durchführen und Ihre AEM-Instanz auf Tomcat 8.5.88 ausgeführt wird, müssen Sie unbedingt das Servlet-Fragment installieren, *bevor* Sie mit der Installation von Service Pack 6.5.15.0 oder höher fortfahren.
...
* Nach der Installation von AEM Service Pack 6.5.14.0 und höher können Benutzende keine Schriftart aus der JEE-Admin-Benutzeroberfläche für PDF-Dokumente auswählen, wenn sie zu `Home` > `Services` > `PDF Generator` > `Adobe PDF Settings` navigieren, da die Schriftartenliste leer angezeigt wird. (FORMS-12095)
...
* Wenn Sie ein adaptives Formular senden, das auf Kernkomponenten basiert, kann das Formular nicht gesendet werden. Nutzen Sie den Umschalter für die Funktion – ft-cq-4324864, um das Problem zu beheben. So wenden Sie einen Umschalter für Funktionen an:
   1. Öffnen Sie die URL: `http://[server]:[port]/system/console/configMgr`
   1. Suchen und öffnen Sie die Konfiguration `com.adobe.granite.toggle.impl.dev.DynamicToggleProviderImpl`.
   1. Geben Sie die Nummer des Umschalters für die Funktion an. Beispiel: ft-cq-4324864.
   1. Speichern Sie die Konfiguration. Die Funktion ist jetzt aktiviert. (FORMS-12789)
...
* Für Struts 2 RCE, ein beliebtes und Open-Source-Web-Anwendungs-Framework für die Entwicklung von Java EE-Web-Anwendungen, wurden kritische Sicherheitslücken gemeldet. Adobe hat [AEM 6.5 Service Pack 19.1 (6.5.19.1) veröffentlicht](/help/forms/using/mitigating-struts-2-rce-vulnerabilities-for-experience-manager-manager-form.md) um die Sicherheitslücke in AEM Forms on JEE zu beheben.
...
* Wenn ein Kunde ein Metadatenschema mit vielen Feldern speichert, überschreitet die mehrteilige/formulardatenbezogene Anfrage des Formularerstellers das Limit maxFileCount, was zu stillen Fehlern führt. (CQDOC-22465)
* Der Vorbefüllungsdienst schlägt mit einer NULL-Zeigerausnahme fehl. (CQDOC-21355)
...
* [Liste der in Experience Manager 6.5.19.0](/help/release-notes/assets/65190_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.19.0](/help/release-notes/assets/65190_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
