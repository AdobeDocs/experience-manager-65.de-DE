...
* Wenn Sie ein adaptives Formular senden, das auf Kernkomponenten basiert, kann das Formular nicht gesendet werden. Wenden Sie den Umschalter für Funktionen an - ft-cq-4324864 , um das Problem zu beheben. So wenden Sie einen Funktionswechsel an:
   1. Öffnen Sie die URL: `http://[server]:[port]/system/console/configMgr`
   1. Suchen und öffnen Sie die `com.adobe.granite.toggle.impl.dev.DynamicToggleProviderImpl` Konfiguration
   1. Geben Sie die Funktions-Rollennummer an. Beispiel: ft-cq-4324864.
...
* Der PDF Generator-Dienst kann die auf dem Server verfügbaren Schriftarten nicht auflisten. Folglich bleibt in der PDFG-Admin-Benutzeroberfläche auf der Seite „Adobe PDF-Einstellungen“ der Schriftauswahlbereich leer, wodurch die Einbettung und Aufhebung der Einbettung ausgewählter Schriftarten verhindert wird. (FORMS-12095)
