...
[!DNL Adobe Experience Manager] 6.5.11.0 enthält neue Funktionen, wichtige von Kundinnen und Kunden angeregte Verbesserungen sowie Leistungs-, Stabilitäts- und Sicherheitsverbesserungen, die seit der Verfügbarkeit der Version 6.5 im April 2019 veröffentlicht wurden. Das Service Pack wird auf [!DNL Adobe Experience Manager] 6.5 installiert.

Die wichtigsten in [!DNL Adobe Experience Manager] 6.5.11.0 eingeführten Funktionen und Verbesserungen sind:
...
Im Folgenden finden Sie die Liste der Fehlerbehebungen in Version [!DNL Experience Manager] 6.5.11.0.
...
* Die Aufzählungsfeldtypen, die einem [!UICONTROL Inhaltsfragmentmodell] hinzugefügt wurde, sind im [!UICONTROL Inhaltsfragmentmodell-Editor] nicht sichtbar (SITES-2391).
...
* Auf der Benutzeroberfläche zum Erstellen eines Videokodierungsprofils in [!DNL Dynamic Media] ist die Option [!UICONTROL Verhältnis für intelligenten Zuschnitt] für bessere Barrierefreiheit gekennzeichnet, sodass die Sprachausgabe sie entsprechend ausgibt.
...
**Behobene Probleme in AEM Forms6.5.11.1**

>[!NOTE]
>
>Wenn Sie noch kein Upgrade auf AEM 6.5.11.0 Forms durchgeführt haben, installieren Sie das AEM Forms 6.5.11.1-Add-on-Paket direkt. Wenn Sie AEM 6.5.11.0 Forms installiert haben, empfiehlt Adobe ein Upgrade auf AEM 6.5.11.1 Forms.

* Übermittlungsaktionen, E-Mail-Versand und das Aufrufen eines AEM-Workflows funktionieren nach der Installation des Add-on-Pakets Forms 6.5.11.0 nicht mehr.
...
* (Nur JEE) Assembler DSC in 6.5.11.0 Patch enthält eine falsche Metainfo-ähnliche Spezifikations- und Impl-Version.
...
## Installieren von 6.5.11.0 {#install}

**Setup-Anforderungen und weitere Informationen**

* Experience Manager 6.5.11.0 erfordert Experience Manager 6.5. Detaillierte Anweisungen finden [&#x200B; in &#x200B;](/help/sites-deploying/upgrade.md)Upgrade-Dokumentation“.
...
* Installieren Sie bei einer Bereitstellung mit MongoDB und mehreren Instanzen Experience Manager 6.5.11.0 mithilfe des Paket-Managers auf einer der Autoreninstanzen.
...
Es gibt zwei Möglichkeiten, [!DNL Experience Manager] 6.5.11.0 automatisch auf einer laufenden Instanz zu installieren:
...
B. Verwenden Sie die [HTTP-API vom Paket-Manager](/help/sites-administering/package-manager.md#package-share). Verwenden Sie `cmd=install&recursive=true`, damit die verschachtelten Pakete installiert werden.
...
* Beim Versuch, entweder Inhaltsfragmente oder Websites/Seiten zu verschieben, zu löschen oder zu veröffentlichen, tritt ein Problem auf, wenn Inhaltsfragmentreferenzen abgerufen werden, da die Hintergrundabfrage fehlschlägt. Die Funktionalität funktioniert daher nicht.
Um einen korrekten Betrieb sicherzustellen, müssen Sie die folgenden Eigenschaften zum Indexdefinitionsknoten `/oak:index/damAssetLucene` hinzufügen (eine Neuindizierung ist nicht erforderlich):
...
In den nachfolgenden Textdokumenten sind die in [!DNL Experience Manager] 6.5.11.0 enthaltenen OSGi- und Inhaltspakete aufgeführt:

* [Liste der in Experience Manager 6.5.11.0enthaltenen OSGi-Pakete](../assets/65110_bundles.txt)
* [Liste der in Experience Manager 6.5.11.0enthaltenen Inhaltspakete](../assets/65110_packages.txt)
