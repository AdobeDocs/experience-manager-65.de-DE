...
* Beim Upgrade von Adobe Experience Manager von 6.5.19.0 auf 6.5.20.0 wurde der Pfad `/libs/cq/graphql/sites/graphiql` gelöscht. (SITES-20098)
...
* Wenn ein(e) Benutzende(r **In PDF drucken** auf AEM Forms Service Pack 15 (6.5.15.0) auf OSGi klickt, schlägt die Client-seitige Validierung fehl, was an den Fehlermeldungen erkennbar wird, die im Fenster „Developer Tools Console“ angezeigt werden. (FORMS-14029)
* Wenn ein(e) Benutzende(r) ein Formular auf AEM 6.5 Forms Service Pack 17 (6.5.17.0) oder Service Pack 18 (6.5.18.0) oder Service Pack 19 (6.5.19.0) sendet, funktioniert die Übersetzung von „Vielen Dank“-Nachrichten nicht ordnungsgemäß. Die Nachrichten sind jedoch korrekt im Wörterbuch übersetzt. (FORMS-13846)
...
* Wenn ein(e) Benutzende(r) in der Umgebung AEM Forms Service Pack 19 (6.5.19.0) die API zum Formatieren von Zahlen aufruft, sind die formatierten Zahlen nicht mit den entsprechenden Gebietsschemata abgestimmt. Daher werden die Währungszeichen nicht korrekt angezeigt. Das Problem bleibt unabhängig davon bestehen, ob der Gebietsschemaparameter auf „de_DE“ oder „en_US“ festgelegt ist. (FORMS-13759)
* Wenn ein(e) Benutzende(r) in der Umgebung AEM Forms Service Pack 19 (6.5.19.0) 16-Bit-PNGs mithilfe des Img2Pdf-PDFG-Dienstes in PDFs konvertiert, schlägt der Vorgang fehl und der Dienst &quot;Acrobat-Bildkonvertierung verwenden“ kann nicht verwendet werden. (FORMS-13754)
* Wenn unter AEM Forms Service Pack 19 (6.5.19.1) ein Benutzer eine vorhandene JobOptions-Datei im Abschnitt Services / PDF Generator / Adobe PDF-Einstellungen der Admin-Benutzeroberfläche von AEM Forms JEE hochlädt, schlägt der Upload fehl. Außerdem wird die folgende Fehlermeldung angezeigt (FORMS-13597):
  `"An error has occurred while processing your request. Please use the breadcrumb links to navigate to another page."`
* Wenn ein(e) Benutzende(r) von AEM Forms Service Pack 15 (6.5.15.0) zu AEM Forms Service Pack (6.5.17.0) oder AEM Forms Service Pack (6.5.19.0) migriert, wird der FD-Schlüssel dupliziert, was dazu führt, dass die Formulare nicht korrekt übersetzt werden. (FORMS-13461)
...
* Auf AEM Forms Workspace unter Service Pack 18 (6.5.18.0) führt ein Kommentar zu einem hochgeladenen Dokument dazu, dass die Dokumentdatei beschädigt wird. (FORMS-13735)
* Auf AEM Forms Service Pack 18 (6.5.18.0), Service Pack 19 (6.5.19.0) oder Service Pack 20 (6.5.20.0) schlägt die Suche fehl, wenn ein(e) Benutzende(r) versucht, im Seitenbereich nach einem adaptiven Formular zu suchen. (FORMS-14117)
...
* Wenn Sie in AEM Forms Service Pack 19 (6.5.19.0) die Dropdown-Liste in einem adaptiven Formular verwenden, wird in den Dropdown-Listen mit Platzhaltertext der Wert `id="emptyValue"` beibehalten. Wenn also ein Formular mehrere Dropdown-Komponenten enthält, verfügt jede über `id="emptyValue"`, was gemäß den ARIA-Richtlinien nicht korrekt ist. (FORMS-13370).
...
* Wenn ein(e) Benutzende(r) einem bestehenden Formular mit AEM Forms Designer in der Umgebung AEM Forms Service Pack 19 (6.5.19.0) eine neue Tabelle hinzufügt, stürzt sie ab. (LC-3921978)
...
* Unter „Tools“ > „Sicherheit“ > „Benutzer“ > &lt;user_name> > „Profile“ wird das Dialogfeld **Benutzereinstellungen bearbeiten** beim Klicken auf „Abbrechen“ nicht geschlossen. (NPR-41793)
...
* [!DNL Experience Manager] 6.5.21.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden Sie in der [Dokumentation zu Upgrades](/help/sites-deploying/upgrade.md). <!-- UPDATE FOR EACH NEW RELEASE -->
* Der Download des Service Packs ist über die [Adobe-Software-Verteilung](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.21.0.zip) verfügbar.
...
* SITES-17934 - Inhaltsfragmente - Vorschau schlägt aufgrund des DoS-Schutzes für große Baumfragmente fehl. Siehe den [KB-Artikel zu den standardmäßigen GraphQL Query Executor-Konfigurationsoptionen](https://experienceleague.adobe.com/de/docs/experience-cloud-kcs/kbarticles/ka-23945)
...
* Wenn ein(e) Benutzende(r) auf AEM Forms Service Pack 20 (6.5.20.0) aktualisiert, das mit dem Forms-Add-on installiert ist, funktionieren Konfigurationen, die den veralteten Cloud-Service für Adobe Analytics mit Authentifizierung über Benutzerberechtigungen verwenden, nicht ordnungsgemäß, was zum Fehler bei der Ausführung von Analytics-Regeln führt. Informationen zum Herunterladen und Installieren des Hotfixes finden Sie im Artikel [Hotfixes für Adobe Experience Manager Forms](/help/release-notes/aem-forms-hotfix.md#hotfix-for-adaptive-forms). (FORMS-15428)
...
* Wenn ein(e) Benutzende(r) getaggte PDFs mit dem Ausgabe-Service auf JEE generiert, wird die Meldung „Warnung vor unangemessener Struktur“ angezeigt. Informationen zum Herunterladen und Installieren des Hotfixes finden Sie im Artikel [Hotfixes für Adobe Experience Manager Forms](/help/release-notes/aem-forms-hotfix.md#hotfix-for-adaptive-forms). (LC-3922038)
...
* [Liste der in Experience Manager 6.5.21.0](/help/release-notes/assets/65210-bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.21.0](/help/release-notes/assets/65210-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
