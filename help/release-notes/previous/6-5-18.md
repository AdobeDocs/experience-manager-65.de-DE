...
* ActiveMQ in AEM wird nicht mehr unterstützt. ActiveMQ wurde für die Kommunikation zwischen zwei AEM-Veröffentlichungsinstanzen verwendet. Adobe empfiehlt, dass Kundinnen und Kunden jetzt einen Lastenausgleich verwenden.
...
* **Dokumentendienste**
   ...
   * Wenn Sie auf AEM 6.5.15.0 Service Pack auf OSGI- und JEE-Umgebungen aktualisieren, funktioniert der Assembler-Dienst bei Verwendung einer bestimmten Vorlage nicht mehr. (FORMS-9355, FORMS-9445, FORMS-9408)
...
* **Adaptive Formulare**.
   ...
   * Beim Arbeiten mit der benutzerdefinierten Fehlerfunktion für den Regeleditor in einem adaptiven Formular treten die folgenden Fehler auf:
      * Wenn Benutzende versuchen, „@param{boolean}“ mit einer Funktion zu verwenden, lässt der Regeleditor nicht zu, dass boolesche Werte an eine Funktion übergeben werden.
      * Wenn Benutzende versuchen, „@param{string}“ mit einer Funktion zu verwenden, kann der Regeleditor die optionalen Werte nicht weitergeben und warnt vor unvollständigen Regeln. (FORMS-9816, FORMS-9815)
   ...
   * Wenn ein(e) Benutzende(r) versucht, ein adaptives Formular mithilfe eines komplexen JSON-Schemas zu erstellen, schlägt dies fehl. Der Fehler tritt wie folgt auf:
   ...
   * Wenn ein adaptives Formular nach der Aktualisierung auf AEM 6.5.16.0 Service Pack neu geladen wird, schlägt das Abrufen von Dateianhängen fehl. (FORMS-8906)
   ...
   * Wenn Sie versuchen, ein Coral-Dialogfeld in AEM 6.5.16.0 Service Pack zu öffnen, wird die Ausnahme `error.log: cannot render resource` generiert. (FORMS-8942)
...
* In AEM 6.5.15.0 sind alle Vanity-URLs, Sling-Aliase und Sling-Zuordnungen nach einem AEM-Neustart beschädigt. (NPR-40420)
...
* [!DNL Experience Manager] 6.5.18.0 erfordert [!DNL Experience Manager] 6.5. Detaillierte Anweisungen finden [ in ](/help/sites-deploying/upgrade.md)Upgrade-Dokumentation“. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Bei einer Bereitstellung mit MongoDB und mehreren Instanzen installieren Sie [!DNL Experience Manager] 6.5.18.0 mit dem Package Manager auf einer der Autoreninstanzen.<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Das OSGi-Bundle `org.apache.jackrabbit.oak-core` Version 1.22.16 oder höher (Zu verwendende Web-Konsole: `/system/console/bundles`).
...
* **Seitenveröffentlichung funktioniert im Seiteneditor nach der Aktualisierung auf Service Pack 18 nicht mehr (6.5.18.0)**

  <!-- https://jira.corp.adobe.com/browse/SITES-15856 REMOVE FOR 6.5.19.0-->

  Nach dem Aktualisieren einer Instanz von AEM 6.5.0.0–6.5.17.0 auf AEM 6.5.18.0 werden Sie beim Klicken auf **Seite veröffentlichen** im Seiteneditor zu einer URL umgeleitet, die nicht existiert.
...
* JDK 11.0.20 wird für die Installation des AEM Forms on JEE-Installationsprogramms nicht unterstützt. Nur JDK 11.0.19 oder frühere Versionen werden für die Installation des Installationsprogramms für AEM Forms on JEE unterstützt. (FORMS-10659)
...
* Nach der Aktualisierung auf die vollständige Installationsumgebung für AEM Forms 6.5.18.0 JBoss Turnkey unter Windows Server 2022 tritt beim Kompilieren des Codes der Output-Client-Anwendung mit Java 11 möglicherweise der folgende Kompilierungsfehler auf:
...
Um das Problem zu beheben, suchen Sie die `<AEM_Forms_Installation_dir>\jboss\bin\standalone.bat`-Datei und aktualisieren Sie `Adobe_Adobe_JAVA_HOME` auf `Adobe_JAVA_HOME` für alle Vorkommen, bevor Sie den Configuration Manager ausführen. (CQDOC-20803)
...
* [Liste der in Experience Manager 6.5.18.0](/help/release-notes/assets/65180_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen OSGi-Bundles
* [Liste der in Experience Manager 6.5.18.0](/help/release-notes/assets/65180_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE --> enthaltenen Inhaltspakete
