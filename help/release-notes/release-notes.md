...
* [hCaptcha](/help/forms/using/integrate-adaptive-forms-hcaptcha.md) und [Cloudflare Turnstile CAPTCHA services](/help/forms/using/integrate-adaptive-forms-turnstile.md): AEM Forms unterstützt die folgenden Captcha-Dienste:
...
* Versionierung adaptiver Formulare:
   * [Mehrere Versionen eines adaptiven Formulars erstellen](/help/forms/using/add-versioning-reviews-comments.md) - Jetzt können Benutzer problemlos Varianten vorhandener Formulare verwalten. Dieser Prozess vereinfacht die Versionskontrolle und erleichtert den Vergleich für die Formularoptimierung - alles innerhalb eines einzigen, optimierten Workflows.
...
* Unterstützung für die Einbettung von Schriftarten in Batch-APIs für interaktive Kommunikation](https://experienceleague.adobe.com/de/docs/experience-manager-65/content/forms/interactive-communications/create-interactive-communication#output-format-print-channel) hinzugefügt - Interaktive Kommunikation unterstützt jetzt das Einbetten von Adobe Ming- und Adobe Myungjo-Schriftarten in PDF, die über die Batch-API generiert wurden. [ Diese Verbesserung sorgt für eines genaues Rendering von Text in generierten Dokumenten, auch wenn Schriftartuntergruppen verwendet werden, und bietet eine verbesserte Unterstützung für mehrsprachige Inhalte in PDF-Ausgaben.
...
* [XDP-Auflösung des Fragments](/help/forms/using/assembler-service.md#resolve-references-on-crx-repository-resolve-references-on-crx-repository) - AEM Forms on OSGi löst jetzt Fragment-XDPs auf, die in Primären XDPs referenziert und im AEM CRX-Repository gespeichert sind.

* [PDF/A-Compliance-Verbesserungen](/help/forms/developing/pdf-a-documents.md#converting-documents-to-pdfa-documents-converting-documents-to-pdf-a-documents) - Jetzt können Benutzer PDF in PDF/A-Formate (1a, 2a, 3a) konvertieren, um sie archivieren zu können, wobei gleichzeitig die Barrierefreiheit gewährleistet und die Einhaltung dieser Normen überprüft wird.

* **Unterstützung für die automatische Skalierung von Schriftarten für statische PDF-Dokumente** - AEM Forms Designer, OutputService und FormsService unterstützen jetzt die automatische Skalierung von Schriftarten für statische PDF. Wenn der Benutzer die Schriftgröße 0 für Text-, numerische, Kennwort- oder Datum/Uhrzeit-Felder festlegt, passt sich die Schriftgröße in diesen Feldern automatisch an, ohne die Gesamtgröße des Felds zu ändern. Um die Funktion zu verwenden, übergeben Benutzer eine Markierung im benutzerdefinierten XCI: `<behaviorOverride>patch-LC-3921991:1</behaviorOverride>`.
...
* Wenn Benutzende in AEM Forms Service Pack 18 (6.5.18.0) versuchen, die Dateien mithilfe des CIF-Protokolls zu drucken, schlägt dies mit folgendem Fehler fehl: (FORMS-16629)
  `ALC-OUT-001-401: Unknown error while printing using CIFS on the Printer: \\\\\\\\NSMVPLUETEST01\\\\TH_Test`.
* Wenn Benutzende von AEM Forms Service Pack 17 (6.5.17.0) auf AEM Forms Service Pack 20 (6.5.20.0) aktualisieren, wird das Symbol für den Regeleditor nicht auf der Ebene des Formular-Containers angezeigt. (FORMS-16430)
* Wenn ein Benutzer ein Upgrade von AEM Forms Service Pack 17 (6.5.17.0) auf AEM Forms Service Pack 21 (6.5.21.0) durchführt, funktioniert der geänderte URL-Pfad für die Übermittlung adaptiver Formulare nicht. (FORMS-15894)
...
* Es wird eine Validierungsregel hinzugefügt, um sicherzustellen, dass das Enddatum nicht vor dem Startdatum liegt, sowie ein benutzerdefiniertes Skript für die Validierungsmeldung. Die Validierung wird jedoch nicht Trigger, wenn das Enddatum vor dem Startdatum liegt. (FORMS-14757)
* Wenn ein Benutzer die Ein-/Ausblenden-Funktion in einer Tabelle in einem adaptiven Formular verwendet, wird die Feldgröße verkleinert. Die Feldgröße wird beim Hinzufügen und Entfernen einer Zeile wieder korrigiert. (FORMS-14756)
* Wenn Benutzende Formulare in AEM Forms Service Pack 19 (6.5.19.0) drucken, werden einige Formulare nicht ordnungsgemäß auf dem Server gerendert, was während des Druckvorgangs zu Fehlern führt. (FORMS-14734)
* Wenn ein Benutzer von AEM Forms Service Pack 15 (6.5.15.0) auf Service Pack 19 (6.5.19.0) aktualisiert, tritt ein Problem auf. Ein benutzerdefiniertes Anzeigemuster, das auf &quot;`num{$zzz,zz9.99}`&quot;festgelegt ist, wird in der Vorschau und der Benutzeroberfläche für Agenten nicht korrekt dargestellt. (FORMS-14694)
...
* In AEM Forms Service Pack 20 (6.5.20.0) stellen Benutzer, die E-Mails mit Anhängen über die Schaltfläche &quot;E-Mail senden&quot;in adaptiven Formularen senden, ein Problem fest. Der Anlagenname wird in der nächsten Zeile anstatt in der Inline-Zeile angezeigt. (FORMS-14426)
...
* Wenn ein Benutzer ein Upgrade von AEMForms-6.5.0-0065 auf AEMForms-6.5.0-0087 bei eigenständigem RHEL8 JBoss®-Setup durchführt, kann er keine Verbindung zum LiveCycle-Service-Container herstellen. (FORMS-15907) *
* In AEM Forms on JEE führt in AEM Workspace die Auswahl eines zuvor gesendeten Formulars, um einen neuen Formularprozess zu starten, zu einem Problem. Forms mit vorausgefüllten Daten überschreibt alle zuvor gesendeten Daten und entfernt manuell ausgefüllte Felder. (FORMS-15376)
* Wen Benutzende in AEM Forms Service Pack 20 (6.5.20.0) mithilfe des PDFG-Dienstes TIFF-Dateien in PDF konvertieren, schlägt der Vorgang mit einem Fehler fehl: (FORMS-14879) ALC-PDG-011-028-Fehler beim Konvertieren der Eingabebilddatei in PDF. com/sun/image/codec/jpeg/JPEGCodec
* Aktualisierung in AEM Forms on JEE-JAR-Dateien: Die Bibliothek `commons-collections:commons-collections:jar` ist jetzt enthalten, um die Auflösung und Funktionalität von Abhängigkeiten für verschiedene AEM Forms JEE-Aufträge zu verbessern, z. B.:
   ...
   * Rights Management Job-Verbesserung zur sicheren Dokumentenverarbeitung und Verbesserung der Rights Management-Funktionen.
...
* Wenn Benutzende in AEM Forms Service Pack 21 (6.5.21.0) nicht standardmäßige Tags zu PDFs hinzufügen, die XMLFM verwenden, entspricht das Dokument nicht den PDF-Spezifikationsanforderungen. (LC-3922484)
* Wenn Benutzende mit dem Ausgabe-Service in AEM Forms Service Pack 20 (6.5.20.0) eine PDF generieren, schlägt dies mit CORBA.COMM_FAILURE fehl und zeigt diesen Fehler an: `15:04:35,973 ERROR [com.adobe.formServer.PA.XMLFormAgentWrapper] (default task-14) ALCOUT-002-013: XMLFormFactory, PAexecute failure: "org.omg.CORBA.COMM_FAILURE"`. Der Dienst wird erfolgreich übergeben, wenn die Barrierefreiheitsrolle &quot;Referenz&quot;aus dem Teilformular der XDP-Vorlage ausgeschlossen ist. Diese Rolle ist jedoch erforderlich, um Konformität mit 508 zu gewährleisten. (LC-3922402)
* Wenn Benutzende ein XFA-Formular in eine AcroForm-PDF konvertieren, schlägt dies fehl. (LC-3922363)
* Wenn Benutzende in AEM Forms Service Pack 19 (6.5.19.0) eine XDP mit den unbenannten Teilformularen erstellt, wird FS_DATA_SOM für unbenannte Teilformulare leer angezeigt. (LC-3922034)

#### Forms Designer {#forms-designer-sp22}

* Wenn Benutzende eine Fragmentbibliothek durch Auswahl eines Fragmentordners in AEM Forms Designer Version 6.5.21.0 öffnen, stürzt es ab. (LC-3922439)
* Wenn Benutzende die 32-Bit-Version von AEM Forms Designer Version 6.5.20.0 deinstallieren und AEM Forms Designer Version 6.5.21.0 installieren, kann Forms Designer nicht gestartet werden. Die Fehlerprotokolle zeigen eine unzureichende Speicherzuordnung für die Java Runtime Environment (JRE) an. (LC-3922404)
* Nachdem Benutzende die AEM Forms Designer-Version 6.5.20.0 installiert haben, wird im Menü nicht die Makros-Option, sondern nur das standardmäßige Makro „Barrierefreiheitsprüfung“ angezeigt, die nicht ausgeführt werden kann. (LC-3922321)
