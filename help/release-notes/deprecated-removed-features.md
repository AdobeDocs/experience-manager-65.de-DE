...
| Adobe Target | Durch das Hinzufügen der Möglichkeit für AEM, mithilfe der [!DNL Adobe I/O]-basierten Adobe Target Standard-API (Rest-API) in AEM 6.5 eine Verbindung zum Adobe Target-Dienst herzustellen, wird die Target Classic-API (XML)-Methode nicht mehr unterstützt. | Konfigurieren Sie die Integration auf [verwenden Sie die neue API](https://helpx.adobe.com/de/experience-manager/kt/sites/using/aem-sites-target-standard-technical-video-understand.html). |
...
| Entwickler | `Hobbes.js`. Adobe plant keine weiteren Verbesserungen am `hobbes.js` Benutzeroberflächentestframework. | Adobe empfiehlt, dass Kunden Selenium-Automatisierung verwenden. |
...
| Entwickler | Verwendung von YUI zum Komprimieren/Minimieren der JavaScript-Client-Bibliotheken: Adobe plant keine weitere Aktualisierung der YUI-Bibliothek. Bis AEM 6.4 wurde JavaScript von YUI standardmäßig mit der Option zum Google Closure Compiler (GCC) minimiert. Ab AEM 6.5 ist GCC der Standard. | Adobe empfiehlt Kunden, die auf AEM 6.5 aktualisieren, zur Implementierung auf GCC zu wechseln |
...
| Entwickler | Entfernen Sie die `customJavaScriptPath`-Unterstützung im HTML Client Library Manager. | Kein Ersatz vorhanden. |
| [!DNL Assets] | Die Funktion zum Abladen von Assets wurde in [!DNL Adobe Experience Manager] 6.5 entfernt. | Es steht kein Ersatz zur Verfügung. |
