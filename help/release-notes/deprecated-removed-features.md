...
| Adobe Search &amp; Promote | Die Integration mit Adobe Search&amp;Promote wird nicht mehr unterstützt. Adobe plant keine weiteren Verbesserungen an der Search &amp; Promote-Integration. Die Adobe Search&amp;Promote-Integration wird weiterhin vollständig unterstützt, ist jedoch veraltet. |  |  |
...
| Adobe Target | Hinzufügen der Möglichkeit für AEM, mithilfe der [!DNL Adobe I/O] Aufgrund der Adobe Target Standard-API (Rest-API) in AEM 6.5 wird die Target Classic-API-Methode (XML) nicht mehr unterstützt. | Integration neu konfigurieren zu [die neue API verwenden](/help/sites-administering/target.md). |  |
...
| Entwickler  | Dialogfeld-Editor für die klassische Benutzeroberfläche in CRXDE Lite. Adobe plant keine weitere Pflege und Aktualisierung des Dialog-Editors für klassische UI, der im Rahmen der Verteilung (Quickstart) bereitgestellt wird. | Es steht kein Ersatz zur Verfügung. |  |
| Forms | Die Integration von AEM Forms mit AEM Mobile wird nicht mehr unterstützt. | Es ist kein Ersatz verfügbar. |  | Entwickler  | Dialogfeld-Editor für die klassische Benutzeroberfläche in CRXDE Lite. Adobe plant keine weitere Pflege und Aktualisierung des Dialog-Editors für klassische UI, der im Rahmen der Verteilung (Quickstart) bereitgestellt wird. | Es steht kein Ersatz zur Verfügung. |  |
